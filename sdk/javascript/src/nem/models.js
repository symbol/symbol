...
class BlockType {
	static NEMESIS = new BlockType(4294967295);

	static NORMAL = new BlockType(1);
...
class Block {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		const transactions = arrayHelpers.read_array_count(view.buffer, Transaction, transactionsCount);
...
class AccountKeyLinkTransaction {
	...
	static deserialize(payload) {
		...
		view.shift(remotePublicKey.size);
...
class Mosaic {
	...
	static deserialize(payload) {
		...
		const mosaicId = MosaicId.deserialize(view.shrinked_buffer(mosaicIdSize));
...
class SizePrefixedMosaic {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const mosaic = Mosaic.deserialize(view.shrinked_buffer(mosaicSize));
...
class MosaicTransferFeeType {
	...
	static fromValue(value) {
		...
		return MosaicTransferFeeType[keys[index]];
	}
...
class SizePrefixedMosaicProperty {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const property = MosaicProperty.deserialize(view.shrinked_buffer(propertySize));
		view.shift(property.size);
...
class MosaicDefinition {
	...
	static deserialize(payload) {
		...
		view.shift(ownerPublicKey.size);
		const idSize = converter.bytesToInt(view.buffer, 4, false);
		view.shift(4);
		// marking sizeof field
		const id = MosaicId.deserialize(view.shrinked_buffer(idSize));
		view.shift(id.size);
		...
		view.shift(descriptionSize);
		const propertiesCount = converter.bytesToInt(view.buffer, 4, false);
		view.shift(4);
		const properties = arrayHelpers.read_array_count(view.buffer, SizePrefixedMosaicProperty, propertiesCount);
		view.shift(properties.map(e => e.size).reduce((a, b) => a + b, 0));
		const levySize = converter.bytesToInt(view.buffer, 4, false);
		view.shift(4);
		let levy = null
		if (0 != levySize) {
			levy = MosaicLevy.deserialize(view.buffer);
			view.shift(levy.size);
		}
	...
	serialize() {
		...
		arrayHelpers.write_array(buffer_, this._properties);
	...
	toString() {
		...
		result += `description: ${this._description /* FIXME */}`;
		result += `properties: ${this._properties.map(e => e.toString())}`;
...
class MosaicDefinitionTransactionBody {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const mosaicDefinition = MosaicDefinition.deserialize(view.shrinked_buffer(mosaicDefinitionSize));
		view.shift(mosaicDefinition.size);
		const rentalFeeSinkSize = converter.bytesToInt(view.buffer, 4, false);
		view.shift(4);
		if (rentalFeeSinkSize !== 40)
			throw RangeError(`Invalid value of reserved field (${rentalFeeSinkSize})`);
		const rentalFeeSink = Address.deserialize(view.buffer);
		view.shift(rentalFeeSink.size);
		const rentalFee = Amount.deserialize(view.buffer);
		view.shift(rentalFee.size);

		const instance = new MosaicDefinitionTransactionBody();
...
class MosaicSupplyChangeTransactionBody {
	...
	static deserialize(payload) {
		...
		const instance = new MosaicSupplyChangeTransactionBody();
		instance._mosaicId = mosaicId;
...
class MultisigAccountModificationType {
	...
	static fromValue(value) {
		...
		return MultisigAccountModificationType[keys[index]];
	}
...
class MultisigAccountModification {
	...
	static deserialize(payload) {
		...
		view.shift(cosignatoryPublicKey.size);
...
class SizePrefixedMultisigAccountModification {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const modification = MultisigAccountModification.deserialize(view.shrinked_buffer(modificationSize));
		view.shift(modification.size);
...
class MultisigAccountModificationTransactionV1 {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		const modifications = arrayHelpers.read_array_count(view.buffer, SizePrefixedMultisigAccountModification, modificationsCount);
		view.shift(modifications.map(e => e.size).reduce((a, b) => a + b, 0));
	...
	serialize() {
		...
		arrayHelpers.write_array(buffer_, this._modifications);
		return buffer_.storage;
	}
	...
	toString() {
		...
		result += `fee: ${this._fee.toString()}`;
		result += `deadline: ${this._deadline.toString()}`;
		result += `modifications: ${this._modifications.map(e => e.toString())}`;
		result += ')';
		return result;
	}
}
...
class Cosignature {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		if (multisigTransactionHashOuterSize !== 36)
		...
		view.shift(4);
		if (multisigTransactionHashSize !== 32)
		...
		view.shift(multisigAccountAddress.size);
...
class SizePrefixedCosignature {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const cosignature = Cosignature.deserialize(view.shrinked_buffer(cosignatureSize));
		view.shift(cosignature.size);
...
class MultisigTransaction {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		// marking sizeof field
		const innerTransaction = NonVerifiableTransactionFactory.deserialize(view.shrinked_buffer(innerTransactionSize));
		view.shift(innerTransaction.size);
		const cosignaturesCount = converter.bytesToInt(view.buffer, 4, false);
		view.shift(4);
		const cosignatures = arrayHelpers.read_array_count(view.buffer, SizePrefixedCosignature, cosignaturesCount);
...
class NamespaceRegistrationTransactionBody {
	...
	static deserialize(payload) {
		...
		if (4294967295 != parentNameSize) {
			...
			view.shift(parentNameSize);
		}

		const instance = new NamespaceRegistrationTransactionBody();
		instance._rentalFeeSink = rentalFeeSink;
		instance._rentalFee = rentalFee;
		instance._name = name;
	...
	serialize() {
		...
		buffer_.write(converter.intToBytes(this._name.length, 4, false)); /* bound: name_size */ 
		buffer_.write(this._name);
		buffer_.write(converter.intToBytes((null !== this._parentName ? this._parentName.length : 4294967295), 4, false)); /* bound: parent_name_size */ 
		if (this.parentName) {
	...
	toString() {
		...
		result += `rentalFee: ${this._rentalFee.toString()}`;
		result += `name: ${this._name /* FIXME */}`;
		result += `parentName: ${this._parentName /* FIXME */}`;
		result += ')';
		return result;
	}
}
...
class TransferTransaction {
	...
	static deserialize(payload) {
		...
		view.shift(4);
		const mosaics = arrayHelpers.read_array_count(view.buffer, SizePrefixedMosaic, mosaicsCount);
	...
	serialize() {
		...
		arrayHelpers.write_array(buffer_, this._mosaics);
		return buffer_.storage;
	}
...
module.exports = {
	...
	BlockType, Block, LinkAction, AccountKeyLinkTransaction, NonVerifiableAccountKeyLinkTransaction, NamespaceId, MosaicId, Mosaic, 
	SizePrefixedMosaic, MosaicTransferFeeType, MosaicLevy, MosaicProperty, SizePrefixedMosaicProperty, MosaicDefinition, 
	MosaicDefinitionTransactionBody, MosaicDefinitionTransaction, NonVerifiableMosaicDefinitionTransaction, MosaicSupplyChangeAction, 
	MosaicSupplyChangeTransactionBody, MosaicSupplyChangeTransaction, NonVerifiableMosaicSupplyChangeTransaction, 
	MultisigAccountModificationType, MultisigAccountModification, SizePrefixedMultisigAccountModification, 
	MultisigAccountModificationTransactionV1, NonVerifiableMultisigAccountModificationTransactionV1, MultisigAccountModificationTransaction, 
	NonVerifiableMultisigAccountModificationTransaction, Cosignature, SizePrefixedCosignature, MultisigTransaction, 
	NamespaceRegistrationTransactionBody, NamespaceRegistrationTransaction, NonVerifiableNamespaceRegistrationTransaction, MessageType, Message, 
