...
def _derive_package_name(output_directory: Path) -> str:
...
def _finalize_imports(source: str, package_name: str) -> str:  # pylint: disable=too-many-locals
...
def generate_files(ast_models, output_directory: Path):
	...
	for ast_model in ast_models:
		...
		sweep_entries.append((ast_model, formatter))
	...
	# the generated per-model sweep test lives in the test tree
	if 'models' == output_directory.name and output_directory.parent.name in ('nem', 'symbol'):
		_write_models_sweep_test(output_directory, package_name, sweep_entries, ast_models)
	else:
		ModelsSweepTestFormatter(sweep_entries, ast_models).render()

	return len(module_entries)


# Test-only imports spliced in after FQN finalization
_SWEEP_TEST_IMPORTS = (
...
def write_sweep_test_file(output_directory, package_name, *, header, imports, filename, body):  # pylint: disable=too-many-arguments
...
def _write_models_sweep_test(output_directory, package_name, sweep_entries, ast_models):
	"""Write the generated per-model sweep test into the mirrored test-tree package."""
	header = (
		'// Auto-generated by sdk/java/generator. Do not edit directly.\n'
		'// Run scripts/run_catbuffer_generator.sh to regenerate.\n')
	sweep = ModelsSweepTestFormatter(sweep_entries, ast_models)
	write_sweep_test_file(
		output_directory, package_name, header=header, imports=_SWEEP_TEST_IMPORTS,
		filename='ModelsSweepTest.java', body=sweep.render())
