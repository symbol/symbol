"""Emits the generated per-descriptor sweep test (the reflection-free).

For each typed descriptor the emitted ``DescriptorsSweepTest.java`` contains explicit ``@Test`` methods that
construct the descriptor with distinct real-valued required arguments
"""
...
class _FieldSample:  # pylint: disable=too-many-instance-attributes
	"""Deterministic sample expressions for one descriptor field (typed / string / expected forms)."""

	def __init__(self, raw_field, resolved, context, type_info, index, *, owner_name):  # pylint: disable=too-many-arguments
	...
	def _make_typed_expr(self):  # pylint: disable=too-many-return-statements
...
class SweepTestFormatter:
	...
	def _render_factory_round_trip_test(self, ast_model, typename, required, optional):  # pylint: disable=too-many-locals
		...
		# factory-built transactions are semantically valid
		lines.extend([
			'',
