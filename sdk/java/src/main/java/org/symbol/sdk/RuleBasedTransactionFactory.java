...
public final class RuleBasedTransactionFactory {
	...
	public RuleBasedTransactionFactory(final Function<Object, Object> typeConverter,
		...
		// NOTE: two deliberate divergences from the JS/Python typeConverterFactory: (1) "handled" is signalled by a
		// non-null return rather than truthiness and (2) there is no reflective ByteArray -> the per-network converters map the
		// facade types explicitly, and the generated pod parse rules rebuild any other ByteArray via Converter.toBytes.
	...
	public void addStructParser(final String name, final Supplier<? extends CatbufferType> factory) {
		rules.put("struct:" + name, descriptor -> {
			...
			// ignore the "type" discriminator that transaction-shaped struct descriptors inject
