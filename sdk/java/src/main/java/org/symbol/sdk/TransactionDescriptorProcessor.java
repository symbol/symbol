...
public final class TransactionDescriptorProcessor {
	...
	void copyTo(final CatbufferType transaction, final List<String> ignoreKeys) {
		for (String key : transactionDescriptor.keySet()) {
			...
			final Object value = lookupValue(key);
			try {
				transaction.setField(key, value);
			} catch (ClassCastException ex) {
				throw new InvalidDescriptorException(String.format("cannot set field %s on %s: value type %s is incompatible", key,
						transaction.getClass().getName(), null == value ? "null" : value.getClass().getName()), ex);
			}
		}
	}
