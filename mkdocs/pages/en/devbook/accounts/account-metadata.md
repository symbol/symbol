...
In this example, the string `alice` is attached an account and then changed to `bob`.
...
The same transaction type handles both adding new metadata and updating existing entries.

Symbol requires these transactions to be inside an <aggregate transaction:> to prevent third parties from attaching
unwanted metadata to an account without permission.
Even when target and signer are the same account, an aggregate is still required to keep the transaction format
uniform.
...
The code adds the embedded account metadata transaction to a <aggregate transaction:>.

Since the signer is modifying their own account, no <cosignatures:> are needed and the aggregate can be of type
complete, which can be signed and announced immediately.
...
To demonstrate updating metadata, the code changes the username from `alice` to `bob` using an embedded account
