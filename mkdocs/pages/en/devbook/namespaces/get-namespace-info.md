...
{{ tutorial.code_snippet(['py:16:20', 'js:10:14']) }}
...
{{ tutorial.code_snippet(['py:22:43', 'js:16:36']) }}
...
{{ tutorial.code_snippet(['py:45:57', 'js:38:49']) }}

Each level in a namespace hierarchy is an independent namespace that can have its own alias.
...
```text hl_lines="3 6 7 8 9 10 11 12 13 14"
...
* **Levels** (lines 9-10): `level0` is the root `symbol` namespace ID (`A95F1F8A96159516`), and `level1` is the `xym`
    subnamespace ID (`E74B99BA41F4AFEE`).

* **End height** (line 12): The value `18446744073709551615` (the maximum 64-bit unsigned integer) means this namespace
    never expires.
