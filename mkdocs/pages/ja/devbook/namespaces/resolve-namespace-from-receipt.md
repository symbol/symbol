...
生のアドレスや [モザイク ID](default:モザイク ID) の代わりにエイリアスを使用するたびに、ネットワークは承認時にエイリアスが指していた実際の値を記録した [レシート](default:レシート) を保存します。ネームスペースの解決を記録するレシートの種類は [解決ステートメント](../../textbook/blocks.md#resolution-statements) と呼ばれます。
...
* ビットが `1` の場合、アドレスは [ネームスペースのアドレスへのリンク](./link-namespace-to-address.md#エイリアスの使用) チュートリアルで説明されているように、エンコードされたネームスペースエイリアスです。
