...
2. `delta`（差分）に追加する絶対単位の数を設定します。
    差分は絶対単位で表されるため、モザイクの [分割性](../../textbook/mosaics.md#divisibility) によって、整数単位への換算が決まることに注意してください。
...
2. `delta` に削除する絶対単位の数を設定します。
    ミントの場合と同様に、差分はモザイクの [分割性](../../textbook/mosaics.md#divisibility) に基づいた絶対単位で表されます。
