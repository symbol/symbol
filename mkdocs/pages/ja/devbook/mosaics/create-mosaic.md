...
[モザイク] (default: モザイク) は、通貨、コレクターズアイテム、アクセス権など、Symbolブロックチェーン上の資産を表します。
...
* モザイクを作成するための [アカウント] (default: アカウント) を、[コードから](../accounts/create-from-private-key.md) または [ウォレットを使用して](../../userbook/wallet/create-account.md) 作成していること。
...
* **Flags (フラグ):** モザイクの動作制限を指定するフラグのセット。
    ...
    * `revokable`: 作成者が任意のアカウントからユニットを回収（取り戻す）できます。
...
| ステップ | 関連ドキュメント |
| ------------------------------------------------------------------------ | ------------------------------------ |
| [モザイクIDを生成する](#モザイク定義トランザクションの構築) | <dy:IdGenerator.generateMosaicId> |
| [モザイクを定義する](#モザイク定義トランザクションの構築) | <dy:SymbolTransactionFactory.create> |
| [モザイク供給量をミントする](#モザイク供給量変更トランザクションの構築) | <dy:SymbolTransactionFactory.create> |
| [モザイクを取得する](#モザイクの取得) | <get:/mosaics/{mosaicId}> |
