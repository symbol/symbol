...
* **Divisibility (分割性):** モザイクがサポートする小数点以下の桁数。
    例えば、値が `2` の場合、各整数単位は 100 ($10^2$) の絶対単位に分割できます。
    詳細については、テキストブックの [分割性](../../textbook/mosaics.md#divisibility) を参照してください。
...
* **検証されたプロパティ** (60-64行目): モザイクがネットワークから取得され、期待通りの供給量、フラグ、分割性、有効期間が確認されました。
