...
Symbol上のすべての[モザイク](default:モザイク)には、供給量、分割性、振る舞いのフラグといったオンチェーンプロパティのセットがあります。
...
設定されていない場合は、テストネット上の<XYM:>の[モザイク ID](default:モザイク ID)である `72C0212E67A08BCE` がデフォルトで設定されます。
...
* **Divisibility (分割性):** モザイクがサポートする[小数点以下の桁数](default:分割性)。
    例えば、XYMの分割性は `6` であり、1 XYMは1,000,000絶対単位に等しいことを意味します。
...
XYM（分割性 `6` ）の場合、 `8325447775994408` 絶対単位の供給量は `8325447775.994408` 全体単位に等しくなります。
...
| ステップ | 関連ドキュメント |
| --- | --- |
| [モザイクプロパティの取得](#_5) | <get:/mosaics/{mosaicId}> |
| [ネームスペースエイリアスの取得](#_7) | <dy:post:/namespaces/mosaic/names>
