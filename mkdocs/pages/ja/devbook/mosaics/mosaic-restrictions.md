...
- **対応する制限キー**: SDK の <dy:Restriction.mosaicRestrictionGenerateKey> 関数を使用して制限名から派生させます。この関数は制限名を SHA3-256 でハッシュ化し、そのハッシュの最初の8バイトを取ります。このアプローチにより、アプリケーションは人間が読みやすい名前を使用しながら、決定論的なキーを生成できます。任意の64ビット数値を制限キーとして直接使用することも可能です。
