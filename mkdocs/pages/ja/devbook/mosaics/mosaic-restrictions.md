...
[モザイク] (default: モザイク) の所有者は、そのモザイクを取引できる [アカウント] (default: アカウント) を制限できます。これらの条件は [モザイク制限] (default: モザイク制限) と呼ばれ、以下の2つの部分で構成されます [cite: 7, 27]。

- [モザイクグローバル制限] (default: モザイクグローバル制限) は、キー・値・関係のタプルとして必要な条件を定義します [cite: 7, 27]。
- [モザイクアドレス制限] (default: モザイクアドレス制限) は、個々のアカウントに対してそれらのキーの値を割り当てます [cite: 7, 27]。
...
制限の設定には複数のトランザクションが必要なため、このチュートリアルではそれらを単一の [アグリゲートコンプリートトランザクション] (default: アグリゲートコンプリートトランザクション) にまとめます。これにより、各トランザクションが個別に承認されるのを待つ必要がなくなります [cite: 4, 83]。



!!! note "メモ" "アカウント制限との違い"

    Symbolは、このチュートリアルで説明するモザイクレベルの制限とは別に、アカウントレベルで定義される [アカウント制限] (default: アカウント制限) もサポートしています [cite: 4, 27]。
...
- 制限を課すための [アカウント] (default: アカウント) を、[コードから](../accounts/create-from-private-key.md) または [ウォレットを使用して](../../userbook/wallet/create-account.md) 作成していること（あるいは提供されているデフォルトアカウントを使用してください） [cite: 4, 71, 75]。
- 制限可能な [モザイク] (default: モザイク) を作成しているか、提供されているデフォルトのものを使用してください。
    [モザイクの作成](./create-mosaic.md) を参照してください [cite: 7, 77]。
- トランザクション手数料を支払うための [XYM] (default: XYM) を入手していること。
    [蛇口 (Faucet) からテストネットの資金を入手する](../accounts/testnet-faucet.md) を参照してください。

さらに、トランザクションがどのようにアナウンスされ承認されるかを理解するために [転送トランザクション](../transactions/transfer.md) を、複数のトランザクションをまとめる方法を理解するために [アグリゲートコンプリートトランザクションの作成](../transactions/complete-aggregate.md) のチュートリアルを復習しておいてください [cite: 4, 78, 83]。
...
- [必要に応じてグローバル制限を有効化](#6)
- [アドレス制限の切り替え](#7)
- 単一のアグリゲートトランザクションに [すべてのトランザクションをまとめる](#8)
- アグリゲートを [アナウンスおよび承認](#9) する
- [テスト転送の送信](#10)
...
- **所有者アカウント**: モザイクを制御し、その制限を設定する責任を持ちます。その秘密鍵は `OWNER_PRIVATE_KEY` 環境変数から64文字の16進数文字列として提供できます [cite: 4, 71]。
- **ターゲットアカウント**: 後にモザイクとの取引許可を受け取るアカウント。そのアドレスは `TARGET_ADDRESS` 環境変数から Symbol テストネットアドレスとして提供できます [cite: 4, 75]。
- **モザイク識別子**: `MOSAIC_ID` から16進数16文字として読み込まれます [cite: 7, 77]。
...
上記で作成されたすべての設定トランザクションは、単一の [アグリゲートコンプリートトランザクション] (default: アグリゲートコンプリートトランザクション) にまとめられます。これにより、ユーザーは各トランザクションが個別に承認されるのを待つ必要がなくなります [cite: 4, 83]。



{{ tutorial.code_snippet(['py:187:196', 'js:204:214']) }}

手数料を支払うのはアグリゲートトランザクションのみであるため、 [埋め込みトランザクション] (default: 埋め込みトランザクション) では `fee` フィールドを使用しません [cite: 4, 7]。
...
最後に、チュートリアルでは標準的な [転送トランザクション] (default: 転送トランザクション) を使用して、所有者アカウントからターゲットアカウントへモザイク1ユニットの送信を試みます [cite: 4, 78]。
...
| ステップ | 関連ドキュメント |
|---------------------------------------------------------------------------------------------|---------------------------------------------|
| [現在のモザイク制限設定の取得](#6) | <get:/restrictions/mosaic> |
| [モザイクグローバル制限の設定](#6) | <ser:MosaicGlobalRestrictionTransactionV1> |
| [アカウントのモザイク制限設定の取得](#7) | <get:/restrictions/mosaic> |
| [モザイクアドレス制限の設定](#7) | <ser:MosaicAddressRestrictionTransactionV1> |
