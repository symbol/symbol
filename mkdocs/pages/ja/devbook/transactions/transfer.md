...
!!! note
    ...
    このチュートリアルでは、ほぼすべてのトランザクションをアナウンスした後に必要となるため、ヘルパーとして定義しています。
...
* **アナウンス応答**（19 行目）: ノードがペイロードを受理したことを示します。
    ただし、この時点ではトランザクションが有効である、またはブロックに取り込まれたことを意味するわけではありません。

* **承認済みステータス**（26 行目）: トランザクションが受理され、ブロックに取り込まれたことを示します。
...
ハッシュは `Transaction hash:` で始まる行に出力されています。
