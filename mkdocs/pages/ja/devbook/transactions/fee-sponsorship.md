---
title: 他のアカウントの代理での手数料支払い
---

# 他のアカウントの代理でのトランザクション手数料の支払い

通常、[ブロックチェーン] (default: ブロックチェーン) を利用するには、[トランザクション] (default: トランザクション) 手数料を支払うためにネットワークのネイティブ通貨を所有している必要があります。
初心者にとって、これは取引所を通じて資金を入手し、[KYC] (default: KYC) 手続きを完了させる必要があることを意味します。

幸いなことに、アプリはユーザーの代わりにトランザクション手数料を負担することで、よりスムーズな体験を提供できます。

Ethereum では、この機能は当初 [EIP-4337](https://eips.ethereum.org/EIPS/eip-4337) などの標準を通じて導入されました。これにより、アカウント抽象化とスポンサー付きトランザクションが可能になりました。
...
!!! warning "注意" "簡潔な例"
    ...
    このチュートリアルで構築されたトランザクションをアナウンスし、承認を確認する方法については、[転送トランザクション](./transfer.md) チュートリアルを参照してください。

## 問題の定義 {#ProblemStatement}
...
## オプション 1: 事前資金供給による手数料 {#Option1}

このアプローチでは、アプリケーション開発者が制御するアカウントが、ユーザーのアカウントに対して *事前資金供給 (prefund)* の転送を行います。

資金が悪用されないことを保証するために、事前資金供給の転送とメッセージの転送の両方が、単一の [アグリゲートトランザクション] (default: アグリゲートトランザクション) 内に組み込まれます。
...
代わりに、 [アグリゲートコンプリートトランザクション] (default: アグリゲートコンプリートトランザクション) を構築してオフチェーン API を通じて開発者の [署名] (default: 署名) を要求するか、 [アグリゲートボンデッドトランザクション] (default: アグリゲートボンデッドトランザクション) を構築してオンチェーンの手段のみで署名を要求することができます。
...
### アグリゲートトランザクション {#AggregateTransaction}

[アグリゲートコンプリートトランザクション] (default: アグリゲートコンプリートトランザクション) は通常通り構築され、トランザクションサイズが判明した時点でその `fee` フィールドが更新されます。
...
その後、前述のようにオンチェーンまたはオフチェーンのいずれかの方法で取得されたアプリケーションアカウントの [連署] (default: マルチシグアカウント) が、 <dy:SymbolFacade.cosignTransaction> を使用して追加されます。

{{ tutorial.code_snippet(['py:57:67', 'js:57:70']) }}

ペイロードが取得できれば、トランザクションをアナウンスし、承認を確認する準備が整います。

## オプション 2: 手数料スポンサーシップ {#Option2}
...
### アグリゲートトランザクション {#AggregateTransaction2}
