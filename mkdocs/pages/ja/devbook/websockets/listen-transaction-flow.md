...
トランザクションは通常通り構築されます。推奨手数料を取得し、トランザクションの記述子からトランザクションを作成して[署名](default: 署名) します。
<dy:SymbolFacade.createTransactionFromTypedDescriptor> は手数料乗数を使用してトランザクション手数料を計算し、期限の長さを受け取ります。

トランザクションに署名すると、それを一意に識別する[ハッシュ](default: ハッシュ) が生成されます。
トランザクションチャネルの通知にはトランザクションハッシュが含まれるため、コードはこのハッシュを保存します。
後で定義するメッセージハンドラは、受信した各ハッシュを保存した値と比較して、このトランザクションに関する通知を識別します。
