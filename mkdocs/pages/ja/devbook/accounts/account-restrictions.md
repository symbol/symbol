...
- 制限を課すための [アカウント] (default: アカウント) を、[コードから](./create-from-private-key.md) または [ウォレットを使用して](../../userbook/wallet/create-account.md) 作成していること（あるいは提供されているデフォルトアカウントを使用してください）。
...
- [必要な鍵の設定](#アカウントの設定)
- [現在のネットワーク状態の取得](#ネットワーク時間と手数料の取得)
- [現在の制限状態の検出](#制限状態の検出)
...
その後、トランザクションは [アナウンスおよび承認](#トランザクションの送信) され、最後に [テスト転送](#テスト転送の送信) が送信されます。
