...
詳細は[下記の説明](#mosaic-ids-and-namespaces)を参照してください。

## モザイクのプロパティ {: #mosaic-properties }
...
:   モザイク数量の小数点以下の桁数を定義するプロパティです。
    可分性が `0` のモザイクは分割不可能であり、全体単位でしか転送できません。
    より大きな値を設定すると、単位を小数に分割できます。

例えば、可分性が `2` の場合、1単位を 100 分割（10^2^）でき、
`0.01` 単位ごとの取引が可能になります。

小数単位は「絶対単位」とも呼ばれ、上記の例では 1 単位が 100 絶対単位で構成されます。
...
モザイク単位が他のアカウントへ配布された後に、モザイクの総供給量を増減できるかを示すプロパティです。
作成者が総供給量のすべてをまだ保持している限り、モザイク単位は常に発行または削除できます。
...
可分性にかかわらず、モザイクの総供給量は **8'999'999'999'000'000** 絶対単位（約 9 × 10^15^）を超えることはできません。
...
* **期限付きモザイク** は、ブロック数で表される有効期限を持ちます。
    ...
    残高はアカウントに残りますが、事実上凍結された状態になり、同じ[モザイクID](default:モザイクID)で
    新しいモザイクを作成することもできません。

!!! warning "有効期限は供給量が 0 のときのみ延長できます"
...
リース手数料の費用はネットワークに照会することで事前に確認でき、[Symbol Desktop Wallet](../userbook/desktop-wallet/install.md) のようなアプリケーションでは通常、この情報が表示されます。

この手数料は作成時に支払う必要があり、返金はされません。
この手数料は、プロトコルとネットワークのメンテナンスのためにモザイクのリース手数料を集める Symbol チームが管理するアカウントである「シンクアカウント」に支払われます。

!!! note "トランザクション手数料とリース手数料"

    モザイクの作成にはトランザクションのアナウンスが必要であり、これにも関連する手数料がかかります。
    ただし、このトランザクション手数料は通常、リース手数料と比較してごくわずかです。

## モザイクIDとネームスペース {: #mosaic-ids-and-namespaces }
...
:   モザイクを定義する際に作成者が選択する、任意の32ビット符号なし整数（0 ～ 4,294,967,295）。
    ...
    同一アカウントでは、既にモザイクで利用しているノンスは再利用することはできません。
...
作成後も、作成者はモザイクの**定義**を変更できますが、総供給量が `0` の間に限られます。

モザイクの[プロパティ](#mosaic-properties)はいずれも変更できます。
供給量の可変性、転送可能性、制限可能性、取り消し可能性、可分性、有効期間が含まれます。

総供給量自体はこれとは別で、モザイクの[供給量の可変性](#supply-mutability)のルールに従って、
ミントやバーンによって調整できます。

## まとめ {: #summary }

以下の表は、モザイクに関する主な数値制限をまとめたものです。

| 制限項目                                   | 値                    |
| ------------------------------------------ | --------------------- |
| 1 アカウントが所有できる最大モザイク数     | 1'000                 |
| モザイクの最大有効期間（無期限でない場合） | 3650 日               |
| モザイクの最大可分性                       | 6                     |
| モザイクあたりの最大絶対単位数       | 8'999'999'999'000'000 |
