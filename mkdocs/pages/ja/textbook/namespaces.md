...
ネームスペースには、[メタデータ](default:メタデータ)を使用して任意の追加データを添付できます。

ネームスペースを登録したアカウントは、その _所有者_ と呼ばれ、
ネームスペースを変更または延長できる唯一のアカウントです。
...
サブネームスペース
:   ルートまたは別のサブネームスペースである親ネームスペースに属するネームスペース。
    「子ネームスペース」とも呼ばれます。
    サブネームスペースは、ルートネームスペースの有効期限が切れると同時に期限切れになります（[有効期限](#duration)を参照）。
...
* 長さは最大 64 文字です。
* ルート名 `symbol`、`symbl`、`xym`、`xem`、`nem`、`user`、`account`、`org`、`com`、`biz`、`net`、`edu`、`mil`、`gov`、`info` はプロトコルによって予約されており、登録できません。

登録後、名前を変更することはできません。

### 有効期間 {: #duration }

ルートネームスペースを登録すると、30 日から 5 年の範囲で固定期間リースされます。
この期間中、所有者は次の操作を実行できます。

* ネームスペースを[モザイク](default:モザイク)または[アカウント](default:アカウント)にリンクし、
  より扱いやすいエイリアスとして使用する。
* ルートネームスペースの下にサブネームスペースを登録する。
...
!!! note "ネームスペースは有効期限前に更新できます"

    [モザイク](default:モザイク)とは異なり、ネームスペースは有効期限前に更新できます。
    これにより、所有者が継続して更新を行う限り、無期限に有効状態を保つことができます。

!!! note "`symbol` ネームスペースは有効期限切れになりません"

    `symbol` ネームスペースは [XYM](default:XYM) をホストしており、永続的に有効で、リースと更新のサイクルの対象外です。
...
[Symbol Desktop ウォレット](../userbook/desktop-wallet/install.md)などのアプリケーションでは通常この情報を表示します。

リース料金は登録または更新時に支払う必要があり、返金不可です。
この料金は、プロトコルとネットワークのメンテナンスのためにネームスペースのリース料金を集める Symbol チームが管理するアカウントである「シンクアカウント」に支払われます。

!!! note "トランザクション手数料とリース料金"
...
* **モザイクにリンクする場合**：モザイクの所有者のみがリンクを設定できます。
...
!!! warning "エイリアスはいつでも変更できます"
    ...
    さらに、ユーザーが入力したエイリアスを受け入れる前に、ウォレットは現在のリンク対象とリンク登録日時を照会し、
    そのエイリアスが意図した宛先を指していることを確認できます。

## 所有権 {: #ownership }

ネームスペースは、ルートネームスペースを登録した[アカウント](default:アカウント)によって管理されます。
