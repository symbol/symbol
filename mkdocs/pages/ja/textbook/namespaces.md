...
ネームスペース
:   [アドレス](default:アドレス)や[モザイク ID](default:モザイク)の代わりに使用できる、人間が読みやすいテキスト文字列。
...
ネームスペースは有限のリソースであり、永続的に所有するのではなく一定期間のリース（賃借）として扱われますが、
リースは更新可能です。

## サブネームスペース

Symbol のネームスペースはインターネットのドメイン名に似た階層構造を持ちます。
各名前はドットで区切られた 1 ～ 3 部構成で、例として `foo`、`foo.bar`、`foo.bar.baz` のようになります。

最初の部分は「ルートネームスペース」と呼ばれ、
それ以降の部分は「サブネームスペース」であり、ルートの下に個別に登録する必要があります。

各ルートネームスペースは最大 256 個のサブネームスペースを持つことができ、
親ネームスペースが期限切れになると同時にそれらも期限切れになります（[有効期間](#_5)参照）。
...
ネームスペースを有効期限前に更新しなかった場合、30 日間の「猶予期間（グレースピリオド）」に入ります。
...
ネームスペースを登録するには、ネットワーク通貨（[XYM](default:モザイク)）によるリース料金の支払いが必要です。
