...
完了待ちの[ボンデッドアグリゲートトランザクション](default:ボンデッドアグリゲートトランザクション)は
メモリ上に保持されます。
...
:   ピアノードは受信したトランザクションとブロックを検証し、
    ...
    さらに[ハーベスタノード](#harvester-nodes)ロールを有効化している場合は、新しいブロックも生成できます。
...
### ハーベスタノード {: #harvester-nodes }

ハーベスタノード
:   ハーベスタノードはブロック生成に貢献し、[ハーベスティング](default:ハーベスティング)報酬を得る資格があります。

!!! note "デュアルノード"

    [API ノード](default:API ノード)と[ハーベスタノード](#harvester-nodes)の両方のロールを有効化したノードは、
