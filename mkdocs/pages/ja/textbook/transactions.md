...
### コンプリートアグリゲートトランザクション {: #complete-aggregate-transactions }

コンプリートアグリゲートトランザクション
...
### ボンデッドアグリゲートトランザクション {: #bonded-aggregate-transactions }
...
署名がすべて揃う前にトランザクションの有効期限（アナウンスから最大 48 時間）が切れた場合、ボンドは没収されますが、
揃えば返還されます。
...
この段階で、必要なすべての署名を収集します（ボンデッドアグリゲートトランザクションを除く）。
...
これは例えば、手数料が低すぎてハーベスタに選ばれなかった場合などに発生します。
...
| **期限** | トランザクションが承認されなかった場合に失効する時刻。最大で 6 時間先まで設定できます（ボンデッドアグリゲートトランザクションは 48 時間先まで）。 |
...
| **期限検証** | トランザクションの期限がすでに経過している、または未来に大きくずれている場合は破棄します。 |
