...
アグリゲートトランザクション
:   アグリゲートトランザクションは、複数の基本トランザクションを結論、
...
連署
:   トランザクションが複数の署名を必要とする場合、それらの署名を「連署（コシグネチャ）」と呼びます。
...
### アグリゲートコンプリートトランザクション {: #complete-aggregate-transactions }

アグリゲートコンプリートトランザクション
:   すべての必要な[連署](default:連署)がすでに付与された状態でネットワークに送信される
    [アグリゲートトランザクション](default:アグリゲートトランザクション)。
    ノードはすぐに検証を行い、ブロックに含めることができます。

このタイプのトランザクションでは、すべての署名を事前にオフチェーンで収集する必要があります。

### アグリゲートボンデッドトランザクション {: #bonded-aggregate-transactions }

アグリゲートボンデッドトランザクション
:   必要な[連署](default:連署)がすべて揃っていない状態でネットワークに送信される
    [アグリゲートトランザクション](default:アグリゲートトランザクション)。
...
アグリゲートボンデッドトランザクションの開始には小額のデポジット（ボンド）が必要です。
署名がすべて揃う前にトランザクションの有効期限が切れた場合、ボンドは没収されますが、
揃えば返還されます。

### 埋め込みトランザクション {: #embedded-transactions }
...
digraph "Transaction Lifecycle" {
    ...
    Finalization [label="7. ファイナライズ", URL="#7-finalization"];
...
[アグリゲートボンデッドトランザクション](default:アグリゲートボンデッドトランザクション)の場合、
すべての署名が揃うまで署名検証は保留されます。

一部のトランザクションタイプでは、追加の検証も必要です。
例えば、転送トランザクションでは送信元アカウントに十分な残高があるか確認します。

詳細な検証項目については、[検証の詳細](#validation-details)を参照してください。
...
未承認トランザクションプール
:   ブロックへの取り込みを待つ検証済みトランザクションの一覧であり、各ノードが保持します。

ピアが伝播されたトランザクションを受け取ると、別のノードによる検証を信頼せず、
自分のプールに追加する前に完全な検証を再実行します。
トランザクションが検証に合格すると、そのピアは自分のピアへ転送し、
ネットワーク全体へ配布されるまで伝播が続きます。

!!! warning "アプリケーションは未承認トランザクションに依存してはいけません"

    未承認トランザクションプール内のトランザクションは、まだブロックに含まれることが保証されていません。
    アプリケーションは、未承認トランザクションを承認済みであるかのように扱ってはいけません。

    例えば、同じ資金を使用し、より高い手数料を支払う別のトランザクションが先に承認される場合があります。
    その場合、未承認トランザクションプール内のトランザクションは無効となり、ネットワークから拒否されます。

### ５．ハーベスティング {: #5-harvesting }
...
トランザクションを含むブロックがコンセンサスによって受け入れられると、そのトランザクションは「承認済み」となります。
...
各ブロックが追加されるごとに確認の層が増え、そのトランザクションが承認済みのままである確率が高まります。

さらに Symbol では、承認済みトランザクションが取り消されないことを保証する
追加の仕組みが存在します。

### ７．ファイナライズ {: #7-finalization }
...
トランザクションがブロックに含まれる前に、各ノードはその有効性を個別に確認します。
使用されるバリデータには、例えば次のものがあります。

| **バリデータ**       | **説明**                                                                                             |
| -------------------- | ---------------------------------------------------------------------------------------------------- |
| **署名検証**         | 署名が有効であり、署名者の公開鍵およびトランザクション内容と一致しているかを確認します。             |
| **手数料検証**       | 最大手数料がノードの最小基準を満たし、署名者に十分な残高があるかを確認します。                       |
| **期限検証**         | トランザクションの期限がすでに経過している場合は破棄します。                                         |
| **タイムスタンプ検証** | 時計操作を防ぐため、タイムスタンプが未来に大きくずれているトランザクションを拒否します。             |
| **ネットワーク検証** | 例えばテストネット用トランザクションをメインネットへ送信した場合など、異なるネットワーク宛てのものを拒否します。 |
| **一意性検証**       | リプレイを防ぐため、直近のチェーン履歴に同じハッシュが存在するトランザクションを拒否します。         |
| **意味的検証**       | トランザクションタイプに基づく論理的整合性を確認します。例：転送トランザクションは送信者の残高が不足していると失敗します。 |

これらの検証のいずれかに失敗したトランザクションは拒否され、ネットワーク上で伝播されません。

## サポートされるトランザクションタイプ {: #supported-transaction-types }

Symbol はさまざまな操作に特化した多くのトランザクションタイプをサポートしています。
すべてのタイプは[共通構造](#common-transaction-structure)を共有し、
...
| `Hash Lock`                                                              | [アグリゲートボンデッドトランザクション](default:アグリゲートボンデッドトランザクション)のアナウンスに必要なデポジットをロックします。 |
| [**ファイナライズ**](#7-finalization )                                    |                                                                                                                                        |
| `Voting Key Link`                                                        | ファイナライズ投票に必要な投票鍵をアカウントにリンクします。                                                                           |
| **[ハーベスティング](default:ハーベスティング)**                         |                                                                                                                                        |
| `Account Key Link`                                                       | リモートまたは委任ハーベスティングを有効化したいすべてのアカウントで必要です。                                                         |
| `Node Key Link`                                                          | 委任ハーベスティングを有効化したいすべてのアカウントで必要です。                                                                       |
| `VRF Key Link`                                                           | ハーベスティングに必要な VRF 公開鍵をアカウントにリンクします。                                                                        |
| **ロック**                                                               |                                                                                                                                        |
| `Secret Lock`                                                            | [HTLC](default:HTLC) のやり取りを開始します。                                                                                          |
| `Secret Proof`                                                           | [HTLC](default:HTLC) のやり取りを完了します。                                                                                          |
| **メタデータ**                                                           |                                                                                                                                        |
...
| `Mosaic Supply Revocation`                                               | モザイク単位を取り消します。                                                                                                           |
