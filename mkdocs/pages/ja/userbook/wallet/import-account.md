...
# 既存のアカウントのインポート {: #importing-an-existing-account }

このページでは、秘密鍵をインポートして既存の [アカウント](default:アカウント) をプロファイルに追加する方法を説明します。  
インポートされたアカウントはプロファイルの [ニーモニックフレーズ](default:ニーモニックフレーズ) から派生したものではないため、個別にバックアップする必要があります。

以下の場合にアカウントのインポートが必要となります。
...
## アカウントのインポート方法 {: #how-to-import-an-account }
...
!!! warning "警告"
    ...
    これらの秘密鍵が [ペーパーウォレット](default:ペーパーウォレット) に含まれるように、必ずすぐにプロファイルをバックアップしてください。
