// Configuration
const NODE_URL = process.env.NODE_URL || 'http://ngl-dual-101.testnet.symboldev.network:3000';

try {
	// Transaction hash to monitor
	// In a real application, you would get this hash after announcing a transaction
	const transactionHash = process.env.TRANSACTION_HASH || '2B6D3B5232E06B9D32682F518C765301FCF9716BFA1EEEF9523653406E04C7EA';
	 ...
	 * @param {number} maxAttempts - Maximum number of polling attempts for confirmation
	 * @param {number} waitSeconds - Seconds to wait between attempts
	 * @returns {boolean} True if transaction was confirmed
	 */
	async function waitForTransactionConfirmation(transactionHash, maxAttempts = 60, waitSeconds = 2) {
		...
		throw new Error(`Transaction ${transactionHash} not confirmed in time`);
	}
