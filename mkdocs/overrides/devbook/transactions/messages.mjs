...
// Configuration
const NODE_URL = process.env.NODE_URL || 'https://001-sai-dual.symboltest.net:3001';
...
try {
	...
	const secretMessage = new TextEncoder().encode('This is a secret message!');
	const encryptedPayload = messageEncoder.encode(recipientPublicKey, secretMessage);
	console.log('Original message:', new TextDecoder().decode(secretMessage));
	console.log('Encrypted payload:', Buffer.from(encryptedPayload).toString('hex'));
	...
	// Sender can decrypt using recipient's public key (to verify what they encrypted)
	const result = messageEncoder.tryDecode(recipientPublicKey, encryptedPayload);
	if (result.isDecoded) {
	...
	// const result = recipientEncoder.tryDecode(senderKeyPair.publicKey, encryptedPayload);

	console.log('\nBoth transactions have been sent successfully!');

} catch (error) {
	console.error('Error:', error.message, '| Cause:', error.cause?.code ?? 'unknown');
}
