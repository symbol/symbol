...
# Configuration
NODE_URL = os.environ.get('NODE_URL', 'http://ngl-dual-101.testnet.symboldev.network:3000')

# Transaction hash to monitor
# In a real application, you would get this hash after announcing a transaction
transaction_hash = os.environ.get('TRANSACTION_HASH', '2B6D3B5232E06B9D32682F518C765301FCF9716BFA1EEEF9523653406E04C7EA')

print(f'Monitoring transaction: {transaction_hash}')

def wait_for_transaction_confirmation(transaction_hash, max_attempts=60, wait_seconds=2):
	"""
	...
	for attempt in range(1, max_attempts + 1):
		try:
			...
			with urllib.request.urlopen(f'{NODE_URL}{status_path}') as response:
			...
			if e.code == 404:
				print(f'  Attempt {attempt}: Transaction status not yet available')
			else:
				raise
