...
def wait_for_confirmation(transaction_hash, label):
	...
	raise Exception(
		f'{label} not confirmed after 60 seconds')
...
try:
	...
	time_path = '/node/time'
	print(
		f'Fetching current network time from {time_path}')
	with urllib.request.urlopen(
		...
		print(
			f'  Network time: {timestamp.timestamp}'
			' ms since nemesis')

	# Fetch recommended fees
	fee_path = '/network/fees/transaction'
	print(
		f'Fetching recommended fees from {fee_path}')
	with urllib.request.urlopen(
			f'{NODE_URL}{fee_path}') as response:
		response_json = json.loads(
			response.read().decode())
		median_mult = (
			response_json['medianFeeMultiplier'])
		minimum_mult = (
			response_json['minFeeMultiplier'])
	...
	definition_tx = facade.transaction_factory.create({
		...
		'signer_public_key':
			signer_key_pair.public_key,
		'deadline':
			timestamp.add_hours(2).timestamp,
		'duration': 0,
		'divisibility': 2,
		'nonce': nonce,
	...
	definition_tx.fee = Amount(
		fee_mult * definition_tx.size)

	mosaic_id = generate_mosaic_id(
		signer_address, nonce)
	print(
	...
	supply_tx = facade.transaction_factory.create({
		'type':
			'mosaic_supply_change_transaction_v1',
		'signer_public_key':
			signer_key_pair.public_key,
		'deadline':
			timestamp.add_hours(2).timestamp,
	...
	supply_tx.fee = Amount(
		fee_mult * supply_tx.size)

	# Sign and generate final payload
	signature = facade.sign_transaction(
		signer_key_pair, supply_tx)
	json_payload = (
		facade.transaction_factory.attach_signature(
			supply_tx, signature))
	print(
	...
	print(json.dumps(
		supply_tx.to_json(), indent=2))

	# Announce and wait for confirmation
	supply_hash = facade.hash_transaction(
		supply_tx)
	print(f'Transaction hash: {supply_hash}')
	announce_transaction(
		json_payload, 'mosaic supply change')
	wait_for_confirmation(
		supply_hash, 'Mosaic supply change')
	...
	with urllib.request.urlopen(
		...
		print(
			'  Divisibility:'
			f' {mosaic_info["divisibility"]}')
		print(
			f'  Duration: {mosaic_info["duration"]}')
