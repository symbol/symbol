...
static uint64_t sha512_ROTR64(uint64_t x, int k) {
	return (x >> k) | (x << (64 - k));
}

static uint64_t sha512_LOAD64_BE(const uint8_t* p) {
	return ((uint64_t)p[0] << 56) | ((uint64_t)p[1] << 48) | ((uint64_t)p[2] << 40) | ((uint64_t)p[3] << 32) | ((uint64_t)p[4] << 24)
		| ((uint64_t)p[5] << 16) | ((uint64_t)p[6] << 8) | ((uint64_t)p[7]);
}

static void sha512_STORE64_BE(uint8_t* p, uint64_t v) {
	p[0] = (uint8_t)(v >> 56);
	p[1] = (uint8_t)(v >> 48);
	p[2] = (uint8_t)(v >> 40);
	p[3] = (uint8_t)(v >> 32);
...
static void sha512_blocks(sha512_state* S, const uint8_t* in, size_t blocks) {
...
static void ed25519_hash_init(sha512_state* S) {
...
static void ed25519_hash_update(sha512_state* S, const uint8_t* in, size_t inlen) {
	size_t blocks, want;
...
static void ed25519_hash_final(sha512_state* S, uint8_t* hash) {
