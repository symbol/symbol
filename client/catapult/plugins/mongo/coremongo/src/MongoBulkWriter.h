...
namespace catapult { namespace mongo { namespace plugins {
	...
	private:
		...
		using AppendOperation = std::function<void (mongocxx::bulk_write&, const TEntity&, uint32_t)>;
	...
	public:
		...
		static std::shared_ptr<MongoBulkWriter> Create(
			...
			std::unique_ptr<uint8_t> pData(reinterpret_cast<uint8_t*>(::operator new(sizeof(MongoBulkWriter))));
			...
			return thread::compose(
					thread::ParallelForPartition(
							...
							[pThis = shared_from_this(), entitiesStart = entities.cbegin(), collectionName, appendOperation, pContext](
								...
								auto index = static_cast<uint32_t>(std::distance(entitiesStart, itBegin));
