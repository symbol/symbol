...
#define TEST_CLASS NodeKeyLinkTransactionTests
...
#undef FIELD
		...
		void AssertTransactionHasExpectedProperties() {
			// Assert:
			EXPECT_EQ(Entity_Type_Node_Key_Link, T::Entity_Type);
...
#undef TRANSACTION_FIELDS
	...
	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(NodeKeyLink)

	// endregion

	TEST(TEST_CLASS, CanCalculateRealSizeWithReasonableValues) {
		// Arrange:
		NodeKeyLinkTransaction transaction;
		transaction.Size = 0;

		// Act:
		auto realSize = NodeKeyLinkTransaction::CalculateRealSize(transaction);

		// Assert:
		EXPECT_EQ(sizeof(NodeKeyLinkTransaction), realSize);
	}
}}
