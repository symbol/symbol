#include "src/plugins/RegisterNamespaceTransactionPlugins.h"
...
#define TEST_CLASS RegisterNamespaceTransactionPluginsTests

	// region TransactionPlugin

	namespace {
		DEFINE_TRANSACTION_PLUGIN_WITH_CONFIG_TEST_TRAITS(RegisterNamespace, NamespaceRentalFeeConfiguration);
	...
	PLUGIN_TEST(CanExtractAccounts) {
		...
		typename TTraits::TransactionType transaction;
		transaction.Duration = ArtifactDuration(1);
	...
	PLUGIN_TEST(RentalFeeIsExtractedFromNonNemesisRootNamespaceWithFiniteLease) {
		...
		pTransaction->Duration = ArtifactDuration(123);
	...
	PLUGIN_TEST(CanExtractRegistrationNotificationsFromRootRegistration) {
		...
		EXPECT_EQ(ArtifactDuration(123), nsRootSub.matchingNotifications()[0].Duration);
