...
namespace validators {
	...
	DECLARE_STATELESS_VALIDATOR(NamespaceName, Notification)
	(uint8_t maxNameSize) {
		return MAKE_STATELESS_VALIDATOR(
			NamespaceName,
			([maxNameSize](const Notification& notification) {
	...
	DECLARE_STATEFUL_VALIDATOR(NamespaceReservedName, Notification)
	(const NameSet& reservedRootNamespaceNames) {
		...
		return MAKE_STATEFUL_VALIDATOR(
			NamespaceReservedName,
			([reservedRootIds](const Notification& notification, const ValidatorContext& context) {
				...
				return reservedRootIds.cend() == reservedRootIds.find(namespaceId) ? ValidationResult::Success
