...
namespace observers {
        ...
        const std::unordered_set<Height, utils::BaseValueHasher<Height>>& forceHeights)
    {
        return MAKE_OBSERVER(
            ExpiredSecretLockInfo,
            model::BlockNotification,
            ([skipHeights, forceHeights](const auto&, ObserverContext& context) {
                ...
                ExpiredLockInfoObserver<cache::SecretLockInfoCache>(
                    context,
                    Expired_Receipt_Type,
                    [height = context.Height,
                        &forceHeights](auto& accountStateCache, const auto& lockInfo, auto accountStateConsumer) {
