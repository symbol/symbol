...
namespace validators {
    ...
    (Height totalVotingBalanceCalculationFixForkHeight, uint64_t votingSetGrouping)
    {
        return MAKE_STATEFUL_VALIDATOR(
            ImportanceBlock,
            ([totalVotingBalanceCalculationFixForkHeight,
                 votingSetGrouping](const Notification& notification, const ValidatorContext& context) {
                ...
                CATAPULT_LOG(debug) << "detected importance block mismatch at " << context.Height << std::endl
                                    << "VotingEligibleAccountsCount" << std::endl
                                    << " + notification " << notification.VotingEligibleAccountsCount << std::endl
                                    << " + statistics   " << statistics.VotingEligibleAccountsCount << std::endl
                                    << "HarvestingEligibleAccountsCount" << std::endl
                                    << " + notification " << notification.HarvestingEligibleAccountsCount << std::endl
                                    << " + statistics   " << statistics.HarvestingEligibleAccountsCount << std::endl
                                    << "TotalVotingBalance" << std::endl
                                    << " + notification " << notification.TotalVotingBalance << std::endl
                                    << " + statistics   " << statistics.TotalVotingBalance;
