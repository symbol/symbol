...
namespace catapult { namespace cache {
	...
	public:
		...
		std::shared_ptr<const state::AccountState> findAccount(const Address& address) const;

		/// Returns account state for an account identified by \a publicKey or \c nullptr if the account was not found.
		std::shared_ptr<const state::AccountState> findAccount(const Key& publicKey) const;
