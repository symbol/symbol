...
#define TEST_CLASS ImportanceViewTests
	...
	namespace {
		...
		constexpr auto Importance_Grouping = 123u;

		struct AddressTraits {
			static auto AddAccount(AccountStateCacheDelta& delta, const Key& publicKey, Height height) {
				return delta.addAccount(model::PublicKeyToAddress(publicKey, Network_Identifier), height);
			}
		};
		...
		void AddAccount(
			...
			auto pState = TTraits::AddAccount(*delta, publicKey, Height(100));
		...
		public:
			const ImportanceView& operator*() { return m_view; }
			const ImportanceView* operator->() { return &m_view; }
		...
		auto CreateAccountStateCache() {
			return std::make_unique<AccountStateCache>(Network_Identifier, Importance_Grouping);
		}
	}
...
#define KEY_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		struct CanHarvestViaMemberTraits {
			static bool CanHarvest(
					const AccountStateCache& cache,
					const Key& publicKey,
					Height height,
					Amount minBalance) {
