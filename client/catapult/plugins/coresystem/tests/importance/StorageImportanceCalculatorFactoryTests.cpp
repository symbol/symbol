...
#pragma pack(pop)
	...
	namespace {
		void AssertWriteCalculatorDelegatesToDecoratedCalculator(ImportanceRollbackMode mode, Importance importanceAdjustment) {
			...
			// Act:
			auto pCalculator = context.createWriteCalculator({ { { addresses[0], Importance_Height }, { Importance(111), Amount(222), 14, 400 } },
	...
	TEST(TEST_CLASS, WriteCalculatorCanOverwriteFile) {
		...
		auto pCalculator = context.createWriteCalculator({ { { addresses1[0], Importance_Height }, { Importance(111), Amount(222), 14, 400 } },
	...
	TEST(TEST_CLASS, ReadCalculatorFailsWhenIndexFileIsNotPresent) {
		...
		// Act + Assert:
		auto pCalculator = context.createReadCalculator({ { { addresses[0], Importance_Height }, { Importance(111), Amount(222), 14, 400 } },
			{ { addresses[1], Importance_Height }, { Importance(123), Amount(432), 22, 200 } },
			{ { addresses[2], Importance_Height }, { Importance(444), Amount(110), 10, 300 } } });
