...
#define TEST_CLASS VrfKeyLinkTransactionPluginTests

	// region test utils

	namespace {
		DEFINE_TRANSACTION_PLUGIN_TEST_TRAITS(VrfKeyLink, 1, 1, )
	}

	DEFINE_BASIC_EMBEDDABLE_TRANSACTION_PLUGIN_TESTS(TEST_CLASS, , , Entity_Type_Vrf_Key_Link)
	...
	namespace {
		...
		void AddCommonExpectations(
			...
			builder.template addExpectation<VrfKeyLinkNotification>([&transaction](const auto& notification) {
	...
	PLUGIN_TEST(CanPublishAllNotificationsInCorrectOrderWhenLinkActionIsLink) {
		...
		test::TransactionPluginTestUtils<TTraits>::AssertNotificationTypes(
				transaction,
				{ KeyLinkActionNotification::Notification_Type, VrfKeyLinkNotification::Notification_Type });
	}
