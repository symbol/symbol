...
#define TEST_CLASS MosaicDefinitionMapperTests
	...
	namespace {
		...
		void AssertMosaicDefinitionData(
			...
			// Assert:
			EXPECT_EQ(parentId, NamespaceId(test::GetUint64(dbTransaction, "parentId")));
		...
		void AssertCanMapTransactionWithName(BlockDuration duration, size_t numExpectedProperties) {
	...
	PLUGIN_TEST(CanMapMosaicDefinitionTransactionWithName_DefaultDuration) {
		// Assert:
		AssertCanMapTransactionWithName<TTraits>(Eternal_Artifact_Duration, 2);
	}

	PLUGIN_TEST(CanMapMosaicDefinitionTransactionWithName_CustomDuration) {
		// Assert:
		AssertCanMapTransactionWithName<TTraits>(BlockDuration(321), 3);
	}
