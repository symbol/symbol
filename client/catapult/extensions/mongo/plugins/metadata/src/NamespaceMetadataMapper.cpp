        ...
        namespace {
            ...
            void Stream(bson_stream::document& builder, const TTransaction& transaction)
            {
                builder << "targetAddress" << ToBinary(transaction.TargetAddress) << "scopedMetadataKey"
                        << static_cast<int64_t>(transaction.ScopedMetadataKey) << "targetNamespaceId" << ToInt64(transaction.TargetNamespaceId)
                        << "valueSizeDelta" << transaction.ValueSizeDelta << "valueSize" << transaction.ValueSize;
