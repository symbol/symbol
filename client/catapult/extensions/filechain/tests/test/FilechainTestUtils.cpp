...
namespace catapult { namespace test {
	...
	config::LocalNodeConfiguration CreateFileChainLocalNodeConfiguration(uint32_t maxDifficultyBlocks, const std::string& dataDirectory) {
		auto config = test::CreateLocalNodeConfigurationWithNemesisPluginExtensions(dataDirectory);
	...
	config::LocalNodeConfiguration CreateStateHashEnabledLocalNodeConfiguration(const std::string& dataDirectory) {
		auto config = CreateFileChainLocalNodeConfiguration(0, dataDirectory);
