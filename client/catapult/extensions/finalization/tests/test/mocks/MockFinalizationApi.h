...
namespace catapult { namespace mocks {
	...
	public:
		...
		/// \note The \a roundRange and \a knownShortHashes parameters are captured.
		thread::future<model::FinalizationMessageRange> messages(
			...
			m_messagesRequests.emplace_back(roundRange, std::move(knownShortHashes));
	...
	private:
		...
		mutable std::vector<std::pair<model::FinalizationRoundRange, model::ShortHashRange>> m_messagesRequests;
	};
}}
