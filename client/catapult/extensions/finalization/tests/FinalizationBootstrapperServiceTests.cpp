...
#define CONSUMER_HOOK_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, MultiRoundMessageAggregatorServiceCountersAreCorrectWhenProcessingSingleRound) {
		...
		RunMultiRoundMessageAggregatorServiceTest([](auto& aggregator, const auto& context, const auto& lastFinalizedHash) {
			...
			aggregator.modifier().add(context.createMessage(VoterType::Large1, { FinalizationPoint(12), 1 }, Height(22), hash));
	...
	TEST(TEST_CLASS, MultiRoundMessageAggregatorServiceCountersAreCorrectWhenProcessingMultipleRounds) {
		...
		RunMultiRoundMessageAggregatorServiceTest([](auto& aggregator, const auto& context, const auto& lastFinalizedHash) {
			...
			auto hash = test::GenerateRandomByteArray<Hash256>();
			aggregator.modifier().add(context.createMessage(VoterType::Large1, { FinalizationPoint(12), 1 }, Height(22), hash));
			aggregator.modifier().add(context.createMessage(VoterType::Large1, { FinalizationPoint(15), 1 }, Height(24), hash));
	...
	TEST(TEST_CLASS, MultiRoundMessageAggregatorServiceCountersAreCorrectWhenProcessingMultipleRoundsWithNewEstimates) {
		...
		RunMultiRoundMessageAggregatorServiceTest([](auto& aggregator, const auto& context, const auto&) {
			...
			aggregator.modifier().add(context.createMessage(VoterType::Large1, { FinalizationPoint(12), 1 }, Height(22), hash1));
			aggregator.modifier().add(context.createMessage(VoterType::Large2, { FinalizationPoint(12), 1 }, Height(22), hash1));

			auto hash2 = test::GenerateRandomByteArray<Hash256>();
			aggregator.modifier().add(context.createMessage(VoterType::Large1, { FinalizationPoint(15), 1 }, Height(24), hash2));
			aggregator.modifier().add(context.createMessage(VoterType::Large2, { FinalizationPoint(15), 1 }, Height(24), hash2));
