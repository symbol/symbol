...
namespace catapult { namespace api {
	...
	namespace {
		struct NetworkTimeTraits {
			static auto Invoke(const TimeSyncApi& api) {
		...
		struct TimeSyncApiTraits {
			static auto Create(const std::shared_ptr<ionet::PacketIo>& pPacketIo) {
				return CreateTimeSyncApi(*pPacketIo);
			}
		};
	}

	DEFINE_REMOTE_API_TESTS_EMPTY_RESPONSE_INVALID(TimeSyncApi, NetworkTime)
}}
