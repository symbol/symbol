...
#define TEST_CLASS UnlockedAccountsTests
    ...
    TEST(TEST_CLASS, AdditionOfAlreadyAddedAccountUpdatesVrfPublicKey)
    {
        ...
        // Act:
        auto result1 = accounts.modifier().add(BlockGeneratorAccountDescriptor(test::CopyKeyPair(signingKeyPair), test::CopyKeyPair(vrfKeyPair1)));
        auto result2 = accounts.modifier().add(BlockGeneratorAccountDescriptor(test::CopyKeyPair(signingKeyPair), test::CopyKeyPair(vrfKeyPair2)));
