...
namespace test {
	...
	model::UnresolvedAddressSet ExtractAddresses(const mocks::MockTransaction& transaction) {
		...
		auto signerAddress = extensions::CopyToUnresolvedAddress(model::PublicKeyToAddress(transaction.SignerPublicKey, networkIdentifier));
		auto recipientAddress = extensions::CopyToUnresolvedAddress(model::PublicKeyToAddress(transaction.RecipientPublicKey, networkIdentifier));
