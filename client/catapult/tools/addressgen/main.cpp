...
namespace catapult { namespace tools { namespace addressgen { namespace {
	...
	public:
		AddressGeneratorTool()
				: AccountTool("Address Generator Tool", AccountTool::InputDisposition::Optional) {
		}

	private:
		void prepareAdditionalOptions(OptionsBuilder& optionsBuilder) override {
			optionsBuilder("count,c", OptionsValue<uint32_t>()->default_value(1), "number of random keys to generate for each pattern");
			optionsBuilder(
					"threads,t",
					OptionsValue<uint32_t>()->default_value(2 * std::thread::hardware_concurrency()),
					"number of threads to use");
			optionsBuilder("showProgress", OptionsSwitch(), "true to print partial matches");
		}
