...
namespace catapult { namespace tools { namespace network { namespace {
	// region node info
	...
	std::string GetName(const IdentityNameMap& identityNameMap, const ionet::Node& node) {
		...
		return identityNameMap.cend() != iter ? iter->second : !node.metadata().Name.empty() ? node.metadata().Name : "?";
	}
	...
	std::string ToString(const ionet::PackedNodeInteractions& interactions) {
		std::ostringstream out;
		out << "successes: " << interactions.NumSuccesses << ", failures: " << interactions.NumFailures;
		return out.str();
	}
	...
	std::string ToString(const ionet::PackedConnectionState& connectionState) {
		std::ostringstream out;
		out << "{ "
			<< "age: " << connectionState.Age << ", ban-age: " << connectionState.BanAge
	...
	private:
		...
		std::vector<thread::future<bool>> getNodeInfoFutures(
			...
			infoFutures.emplace_back(pApi->nodeInfo().then([nodeIdentity, &nodeInfo](auto&& nodeFuture) {
				return UnwrapFutureAndSuppressErrors(
						"querying node info",
						std::move(nodeFuture),
						[nodeIdentity, &nodeInfo](const auto& remoteNode) {
		...
		size_t processNodeInfos(const std::vector<NodeInfoPointer>& nodeInfos) override {
			...
			return utils::Sum(nodeInfos, [](const auto& pNodeInfo) { return Key() == pNodeInfo->Remote.identity().PublicKey ? 1u : 0; });
		}
	};
}}}}
