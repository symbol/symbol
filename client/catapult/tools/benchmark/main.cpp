...
namespace catapult { namespace tools { namespace benchmark { namespace {
	...
	public:
		...
		void prepareOptions(OptionsBuilder& optionsBuilder, OptionsPositional&) override {
			optionsBuilder("num threads,t", OptionsValue<uint32_t>(m_numThreads)->default_value(0), "number of threads");
			optionsBuilder("num partitions,p", OptionsValue<uint32_t>(m_numPartitions)->default_value(0), "number of partitions");
			optionsBuilder(
					"ops / partition,o",
					OptionsValue<uint32_t>(m_opsPerPartition)->default_value(1000),
					"number of operations per partition");
			optionsBuilder("data size,s", OptionsValue<uint32_t>(m_dataSize)->default_value(148), "size of the data to generate");
		}
		...
		int run(const Options&) override {
			...
			CATAPULT_LOG(info) << "num threads (" << m_numThreads << "), num partitions (" << m_numPartitions << "), ops / partition ("
							   << m_opsPerPartition << "), data size (" << m_dataSize << ")";
			...
			RunParallel("Signature", *pPool, entries, [&keyPair](auto& entry) { crypto::Sign(keyPair, entry.Data, entry.Signature); });
	...
	private:
		...
		uint64_t RunParallel(const char* testName, thread::IoThreadPool& pool, std::vector<BenchmarkEntry>& entries, TAction action) const {
			...
			CATAPULT_LOG(info) << (0 == opsPerSecond ? "???" : std::to_string(opsPerSecond)) << " ops/s "
