...
namespace catapult { namespace tools { namespace testvectors { namespace {
	// region helpers
	...
	void AssertSize(const std::string& name, size_t expectedSize, const RawString& str, size_t testCaseNumber) {
		if (expectedSize != str.Size) {
			...
			out << "invalid data " << name << " in test case " << testCaseNumber << " has invalid size: " << str.Size
	...
	auto Concatenate(const crypto::AesGcm256::Tag& tag, const crypto::AesGcm256::IV& iv, const std::vector<uint8_t>& encryptedPayload) {
	...
	public:
		...
		void prepareOptions(OptionsBuilder& optionsBuilder, OptionsPositional& positional) override {
			optionsBuilder("vectors,v", OptionsValue<std::string>(m_vectorsDirectory)->required(), "path to test-vectors directory");

			optionsBuilder(
					"inclusion-filter,i",
					OptionsValue<std::vector<uint16_t>>(m_inclusionFilter)->multitoken()->default_value({ 0, 1, 2, 3, 4, 5, 6 }, "all"),
