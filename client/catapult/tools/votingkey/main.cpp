...
namespace catapult { namespace tools { namespace votingkey { namespace {
	...
	public:
		...
		void prepareOptions(OptionsBuilder& optionsBuilder, OptionsPositional&) override {
			...
			optionsBuilder("startEpoch,b", OptionsValue<uint32_t>(m_startEpoch)->default_value(1), "voting key start epoch");
			optionsBuilder("endEpoch,e", OptionsValue<uint32_t>(m_endEpoch)->default_value(26280), "voting key end epoch");
			optionsBuilder("secret,s", OptionsValue<std::string>(m_secretKey), "root secret key (testnet only, don't use in production)");
		}

		int run(const Options&) override {
			crypto::SecureRandomGenerator generator;
			auto keyPair =
					BmKeyPair::FromPrivate(BmKeyPair::PrivateKey::Generate([&generator] { return static_cast<uint8_t>(generator()); }));
	...
	private:
		BmPublicKey generateTree(crypto::VotingKeyPair&& keyPair) {
			...
			crypto::BmOptions options{ ToKeyIdentifier(FinalizationEpoch(m_startEpoch), model::FinalizationStage::Prevote),
