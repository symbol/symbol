...
#pragma pack(push, 1)
	...
	struct TransactionContainer : public TEntityHeader {
		/// Gets a container wrapping the transactions contained in this container with the
		...
		/// Gets a container wrapping the const transactions contained in this container with the
		/// desired error policy (\a errorPolicy).
		auto Transactions(EntityContainerErrorPolicy errorPolicy = EntityContainerErrorPolicy::Throw) const {
			return MakeContiguousEntityContainer(TransactionsPtr(), GetTransactionPayloadSize(*this), errorPolicy);
		}

		/// Gets a pointer to transactions contained in this container.
		...
		/// Gets a const pointer to transactions contained in this container.
