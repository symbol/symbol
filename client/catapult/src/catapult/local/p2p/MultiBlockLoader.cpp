...
namespace catapult { namespace local { namespace p2p {
	...
	namespace {
		...
		public:
			SkipTransientStatePredicate(const model::Block& lastBlock, const model::BlockChainConfiguration& config)
					...
					, m_inflectionHeight(DifferenceOrZero(
							lastBlock.Height.unwrap(),
							config.MaxDifficultyBlocks) + 1)
			{}
	...
	private:
		using NotifyProgressFunc = std::function<void (Height, Height)>;
	...
	private:
		...
		const LocalNodeStateRef& m_stateRef;
	};

	model::ChainScore LoadBlockChain(
			const BlockDependentEntityObserverFactory& observerFactory,
			const LocalNodeStateRef& stateRef) {
		BlockChainLoader loader(observerFactory, stateRef);
