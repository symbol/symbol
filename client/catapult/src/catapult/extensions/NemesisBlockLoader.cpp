...
namespace extensions {
	...
	namespace {
		...
		void OutputNemesisBalance(std::ostream& out, MosaicId mosaicId, Amount amount, char special = ' ') {
			out << std::endl
				<< "      " << special << ' ' << utils::HexFormat(mosaicId) << ": " << amount;
		}
	...
	void NemesisBlockLoader::validateStatefulAndObserve(
		...
		executionConfig.pNotificationPublisher = model::CreateNemesisNotificationPublisher(m_pluginManager.createNotificationPublisher(), m_publisherOptions);
