...
namespace extensions {
    ...
    DECLARE_OBSERVER(NemesisFunding, Notification)
    (const Address& nemesisAddress, NemesisFundingState& fundingState)
    {
        return MAKE_OBSERVER(
            NemesisFunding,
            Notification,
            ([&nemesisAddress, &fundingState](const Notification& notification, const observers::ObserverContext& context) {
                ...
                // assume that all mosaics are funded in same way (i.e. nemesis cannot have mix of implicitly and explicitly funded
                // mosaics)
                if (NemesisFundingType::Unknown == fundingState.FundingType) {
                    fundingState.FundingType = Amount() != senderState.Balances.get(mosaicId) ? NemesisFundingType::Explicit
