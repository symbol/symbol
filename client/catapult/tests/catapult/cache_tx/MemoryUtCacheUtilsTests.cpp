...
#define GET_FIRST_TRAITS_BASED_TEST(TEST_NAME)                                   \
	...
	TEST(TEST_CLASS, GetFirstTransactionInfoPointersAppliesSorting_SortedFiltered) {
		...
		auto transactionInfos = GetFirstTransactionInfoPointers(utCacheView, 3, CountAsOne, CompareReverseOrder, [](const auto&) { return true; });
	...
	TEST(TEST_CLASS, GetFirstTransactionInfoPointersAppliesStableSorting_SortedFiltered) {
		...
		// Act: apply a reverse sort
		auto transactionInfos = GetFirstTransactionInfoPointers(utCacheView, 4, CountAsOne, CompareReverseOrder, [](const auto&) { return true; });
	...
	TEST(TEST_CLASS, GetFirstTransactionInfoPointersAppliesFiltering_SortedFiltered) {
		...
		// Act: filter odd deadline txes
		auto transactionInfos = GetFirstTransactionInfoPointers(utCacheView, 3, CountAsOne, CompareNaturalOrder, [](const auto& transactionInfo) {
			return 0 == transactionInfo.pEntity->Deadline.unwrap() % 2;
		});
	...
	TEST(TEST_CLASS, GetFirstTransactionInfoPointersAppliesSortingAndFiltering_SortedFiltered) {
		...
		auto transactionInfos = GetFirstTransactionInfoPointers(utCacheView, 3, CountAsOne, CompareReverseOrder, [](const auto& transactionInfo) {
			return 0 == transactionInfo.pEntity->Deadline.unwrap() % 2;
		});
