...
#define GET_FIRST_TRAITS_BASED_TEST(TEST_NAME) \
	...
	// region GetMatchingTransactionInfoPointers (Signature)

	TEST(TEST_CLASS, GetMatchingTransactionInfoPointers_ReturnsNoneWhenNoMatching) {
		// Arrange:
		auto pUtCache = test::CreateSeededMemoryUtCache(10);
		auto utCacheView = pUtCache->view();

		auto signatures = test::GenerateRandomDataVector<Signature>(5);

		// Act: request no matching signatures
		auto transactionInfos = GetMatchingTransactionInfoPointers(utCacheView, signatures);

		// Assert:
		EXPECT_TRUE(transactionInfos.empty());
	}

	TEST(TEST_CLASS, GetMatchingTransactionInfoPointers_ReturnsSomeWhenSomeMatching) {
		// Arrange:
		auto pUtCache = test::CreateSeededMemoryUtCache(10);
		auto utCacheView = pUtCache->view();
		auto allTransactionInfos = test::ExtractTransactionInfos(utCacheView, 10);
		...
		auto signatures = test::GenerateRandomDataVector<Signature>(5);
		for (const auto& pair : std::initializer_list<std::pair<size_t, size_t>>{ { 0, 9 }, { 1, 1 }, { 4, 4 } })
			signatures[pair.first] = allTransactionInfos[pair.second]->pEntity->Signature;

		// Act: request some matching signatures
		auto transactionInfos = GetMatchingTransactionInfoPointers(utCacheView, signatures);

		// Assert: importantly, order is preserved
		ASSERT_EQ(3u, transactionInfos.size());
		test::AssertEqual(*allTransactionInfos[9], *transactionInfos[0], "transaction at 0");
		test::AssertEqual(*allTransactionInfos[1], *transactionInfos[1], "transaction at 1");
		test::AssertEqual(*allTransactionInfos[4], *transactionInfos[2], "transaction at 2");
	}

	TEST(TEST_CLASS, GetMatchingTransactionInfoPointers_ReturnsAllWhenAllMatching) {
		// Arrange:
		auto pUtCache = test::CreateSeededMemoryUtCache(10);
		auto utCacheView = pUtCache->view();
		auto allTransactionInfos = test::ExtractTransactionInfos(utCacheView, 10);
		...
		auto signatures = test::GenerateRandomDataVector<Signature>(5);
		for (const auto& pair : std::initializer_list<std::pair<size_t, size_t>>{ { 0, 9 }, { 1, 1 }, { 2, 4 }, { 3, 8 }, { 4, 7 } })
			signatures[pair.first] = allTransactionInfos[pair.second]->pEntity->Signature;

		// Act: request all matching signatures
		auto transactionInfos = GetMatchingTransactionInfoPointers(utCacheView, signatures);
		...
		test::AssertEqual(*allTransactionInfos[4], *transactionInfos[2], "transaction at 2");
		test::AssertEqual(*allTransactionInfos[8], *transactionInfos[3], "transaction at 3");
		test::AssertEqual(*allTransactionInfos[7], *transactionInfos[4], "transaction at 4");
	}
