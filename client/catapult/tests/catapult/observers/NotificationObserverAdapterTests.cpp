...
#define TEST_CLASS NotificationObserverAdapterTests
	...
	TEST(TEST_CLASS, ExtractsAndForwardsNotificationsFromEntity) {
		...
		RunTest([](const auto& adapter, const auto& observer) {
			...
			EXPECT_EQ(model::Core_Transaction_Fee_Notification, observer.notificationTypes()[3]);
			EXPECT_EQ(model::Core_Balance_Debit_Notification, observer.notificationTypes()[4]);

			// - mock transaction notifications
			EXPECT_EQ(model::Core_Register_Account_Public_Key_Notification, observer.notificationTypes()[5]);
			EXPECT_EQ(mocks::Mock_Observer_1_Notification, observer.notificationTypes()[6]);
			EXPECT_EQ(mocks::Mock_All_1_Notification, observer.notificationTypes()[7]);
			EXPECT_EQ(mocks::Mock_Observer_2_Notification, observer.notificationTypes()[8]);
			EXPECT_EQ(mocks::Mock_All_2_Notification, observer.notificationTypes()[9]);
