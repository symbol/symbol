...
#define STREAM_ROUNDTRIP_TEST(STREAM_NAME, WRITE_POLICY, READ_POLICY) \
	TEST(STREAM_NAME##Tests, RunRoundtripTest_##WRITE_POLICY##_##READ_POLICY) { \
		test::RunRoundtripTest<STREAM_NAME##Context, test::WRITE_POLICY, test::READ_POLICY>(); \
	}
...
#define DEFINE_STREAM_TESTS(STREAM_NAME) \
	TEST(STREAM_NAME##Tests, CannotReadMoreThanWritten) { \
		test::CannotReadMoreThanWritten<STREAM_NAME##Context>(); \
	} \
	\
