...
#define TEST_CLASS BlockElementSerializerTests
	...
	namespace {
		...
		WriteTestContext PrepareWriteTestContext(uint32_t numTransactions, uint32_t numSubCacheMerkleRoots) {
			...
			auto hashOffset = 2u;
			for (auto& transactionElement : context.pBlockElement->Transactions) {
				transactionElement.EntityHash = context.Hashes[hashOffset];
				transactionElement.MerkleComponentHash = context.Hashes[hashOffset + 1];
				hashOffset += 2;
			}
		...
		std::vector<Hash256> ExtractHashes(const std::vector<uint8_t>& buffer, size_t blockSize) {
			...
			hashes.insert(hashes.end(), pHash, pHash + numTransactionHashes);

			// 3. extract sub cache merkle roots
			pData += sizeof(uint32_t) + numTransactionHashes * Hash256_Size;
	...
	TEST(TEST_CLASS, CanWriteBlockElement) {
		...
		// Act:
		WriteBlockElement(outputStream, *context.pBlockElement);
