...
// For gcc 12 and greater add the namespace manually
	...
	void AssertExceptionInformation(const TException& ex, const ExpectedDiagnostics<TTraits>& expected) {
		...
		std::vector<std::string> expectedDiagLines { "Throw in function " + ConvertToExceptionFunctionName(expected.FunctionName),
...
namespace {
	...
	struct FileIoErrorTraits {
		...
		static constexpr auto Exception_Fqn = CLASSPREFIX "catapult::catapult_error<" CLASSPREFIX "catapult::catapult_error<" CLASSPREFIX "std::runtime_error> >";
	};
}
...
#define EXCEPTION_TRAITS_BASED_TEST(TEST_NAME)                            \
	...
	TEST(TEST_CLASS, TEST_NAME##_RuntimeError) {                          \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<RuntimeErrorTraits>();    \
	}                                                                     \
	TEST(TEST_CLASS, TEST_NAME##_InvalidArgument) {                       \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<InvalidArgumentTraits>(); \
	}                                                                     \
	TEST(TEST_CLASS, TEST_NAME##_OutOfRange) {                            \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<OutOfRangeTraits>();      \
	}                                                                     \
	TEST(TEST_CLASS, TEST_NAME##_FileIoErrorTraits) {                     \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<FileIoErrorTraits>();     \
	}                                                                     \
	template <typename TTraits>                                           \
	void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)()
...
#define TAG_TRAITS_BASED_TEST(TEST_NAME)                               \
	template <typename TTraits>                                        \
	void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)();                    \
	TEST(TEST_CLASS, TEST_NAME##_Pod) {                                \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<PodTagTraits>();       \
	}                                                                  \
	TEST(TEST_CLASS, TEST_NAME##_Atomic) {                             \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<AtomicTagTraits>();    \
	}                                                                  \
	TEST(TEST_CLASS, TEST_NAME##_BaseValue) {                          \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<BaseValueTagTraits>(); \
	}                                                                  \
	template <typename TTraits>                                        \
	void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)()
