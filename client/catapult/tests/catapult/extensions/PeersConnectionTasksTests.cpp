...
#define TEST_CLASS PeersConnectionTasksTests
	...
	namespace {
		...
		void AssertCanCreateSelectorSettings(
			ionet::IpProtocol expectedProtocols,
	...
	namespace {
		...
		void RunConnectPeersTask(
			...
			auto cache = test::CreateEmptyCatapultCache();
			auto settings = CreateDefaultSelectorSettings(cache, container, serviceId, ionet::IpProtocol::IPv4, ionet::NodeRoles::Api);
			auto task = selector ? CreateConnectPeersTask(settings, packetWriters, selector) : CreateConnectPeersTask(settings, packetWriters);
