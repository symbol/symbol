...
#define TEST_CLASS StaticNodeRefreshServiceTests
	...
	namespace {
		void AssertRefreshSucceedsWhenThereAreMultipleResolvableStaticNodes(const std::string& endpointHost) {
			...
			// - set up static nodes
			auto staticNodes = std::vector<ionet::Node> { CreateNamedLocalHostNode(pAcceptorServer->publicKeyAt(0), endpointHost, 0, "alice"),
			...
			test::RunTaskTestPostBoot(context, 1, Task_Name, [&nodes, &pAcceptorServer, &endpointHost](const auto& task) {
				...
				auto expectedContents = test::BasicNodeDataContainer { { pAcceptorServer->publicKeyAt(0), "alice", ionet::NodeSource::Static },
