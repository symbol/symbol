#include "catapult/local/ReverseNotificationObserverAdapter.h"
...
#define TEST_CLASS ReverseNotificationObserverAdapterTests
	...
	namespace {
		...
		void RunTest(TRunTestFunc runTest) {
			...
			auto pAdapter = std::make_unique<ReverseNotificationObserverAdapter>(*pRegistry, std::move(pObserver));
	...
	TEST(TEST_CLASS, ExtractsAndForwardsNotificationsFromEntityInReverseOrder) {
		...
		RunTest([](const auto& adapter, const auto& observer) {
			...
			EXPECT_EQ(model::Core_Register_Account_Public_Key_Notification, observer.notificationTypes()[6]);
			EXPECT_EQ(model::Core_Transaction_Notification, observer.notificationTypes()[5]);
