...
#pragma pack(push, 1)
		...
		public:
			explicit CustomReceipt(const std::array<uint8_t, N>& payload)
				: Payload(payload) {
...
#pragma pack(pop)
		...
		std::vector<uint8_t> CreateExpectedSerializedDataForHashTests(const std::vector<uint8_t>& receiptsData) {
			std::vector<uint8_t> allData { 0x01, 0x00, // version
				0x43, 0xE1, // type
	...
	TEST(TEST_CLASS, CanCalculateHashWithMultipleDuplicateAttachedReceipts) {
		...
		// Assert:
		Hash256 expectedHash;
		auto expectedSerializedData = CreateExpectedSerializedDataForHashTests({ 0x02, 0x00, 0x04, 0x00, 0xAB, 0xFA, 0xCE, 0x55, 0x02, 0x00, 0x04, 0x00,
	...
	TEST(TEST_CLASS, CanCalculateHashWithMultipleAttachedReceipts_InOrder) {
		...
		// Assert:
		Hash256 expectedHash;
		auto expectedSerializedData = CreateExpectedSerializedDataForHashTests({ 0x02, 0x00, 0x03, 0x00, 0x39, 0x62, 0x19, 0x02, 0x00, 0x04, 0x00, 0xAB,
