...
#define TEST_CLASS TreeNodeTests
	...
	namespace {
		...
		TreeNodePathTestCase GetEvenTreeNodePathTestCase(PathType type) {
			...
			case PathType::Long:
				return {
					TreeNodePath(std::vector<uint8_t> { 0x64,
						0x6F,
						0x67,
						0x00,
						0x19,
						...
						0xFC,
						0xEE }),
					{ 0x20, 0x64, 0x6F, 0x67, 0x00, 0x19, 0xA3, 0xBC, 0x74, 0x24, 0x82, 0x3C, 0x98, 0x79, 0x25, 0xD4, 0x60, 0xFC, 0xEE }
				};

			case PathType::Ending_On_Boundary:
				return { TreeNodePath(std::vector<
							 uint8_t> { 0x64, 0x6F, 0x67, 0x00, 0x19, 0xA3, 0xBC, 0x74, 0x24, 0x82, 0x3C, 0x98, 0x79, 0x25, 0xD4 }),
					{ 0x20, 0x64, 0x6F, 0x67, 0x00, 0x19, 0xA3, 0xBC, 0x74, 0x24, 0x82, 0x3C, 0x98, 0x79, 0x25, 0xD4 } };
			}
		...
		TreeNodePathTestCase GetOddTreeNodePathTestCase(PathType type) {
			...
			case PathType::Long:
				return { TreeNodePath(std::vector<uint8_t> { 0x64,
										  0x6F,
										  0x67,
										  0x00,
										  0x19,
										  ...
										  0xEE })
							 .subpath(0, 35),
					{ 0x36, 0x46, 0xF6, 0x70, 0x01, 0x9A, 0x3B, 0xC7, 0x42, 0x48, 0x23, 0xC9, 0x87, 0x92, 0x5D, 0x46, 0x0F, 0xCE } };
			...
			case PathType::Ending_On_Boundary:
				return { TreeNodePath(std::vector<uint8_t> { 0x64,
										  ...
										  0x60 })
							 .subpath(0, 31),
					{ 0x36, 0x46, 0xF6, 0x70, 0x01, 0x9A, 0x3B, 0xC7, 0x42, 0x48, 0x23, 0xC9, 0x87, 0x92, 0x5D, 0x46 } };
			}
...
#define BRANCH_LINK_TEST(TEST_NAME)                                \
	...
	TEST(TEST_CLASS, TEST_NAME##_HashLink) {                       \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<HashLinkTraits>(); \
	}                                                              \
	TEST(TEST_CLASS, TEST_NAME##_NodeLink) {                       \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<NodeLinkTraits>(); \
	}                                                              \
	template <typename TTraits>                                    \
	void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)()
