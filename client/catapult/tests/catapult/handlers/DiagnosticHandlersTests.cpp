...
#define TEST_CLASS DiagnosticHandlersTests
	...
	namespace {
		...
		ionet::ConnectionState CreateConnectionState(uint32_t age, uint32_t numAttempts, uint32_t numSuccesses, uint32_t numFailures) {
	...
	TEST(TEST_CLASS, DiagnosticNodesHandler_WritesNodeInformationInResponseToValidRequest_SingleActiveNode) {
		...
		ionet::NodeContainer nodeContainer;
		{
			...
			modifier.provisionConnectionState(ionet::ServiceIdentifier(123), keys[1]) = CreateConnectionState(0, 2, 3, 4);
			modifier.provisionConnectionState(ionet::ServiceIdentifier(987), keys[1]) = CreateConnectionState(49, 64, 36, 25);
		}
		...
		AssertDiagnosticNodesHandlerWritesCountsInResponseToValidRequest(nodeContainer, expectedPacketSize, [&keys](const auto& context) {
			...
			AssertConnectionState(nodeInfo, ionet::ServiceIdentifier(123), 0, 2, 3, 4);
			AssertConnectionState(nodeInfo, ionet::ServiceIdentifier(987), 49, 64, 36, 25 );
		});
	}
	...
	TEST(TEST_CLASS, DiagnosticNodesHandler_WritesNodeInformationInResponseToValidRequest_MultipleActiveNodes) {
		...
		ionet::NodeContainer nodeContainer;
		{
			...
			modifier.provisionConnectionState(ionet::ServiceIdentifier(123), keys[0]) = CreateConnectionState(7, 6, 5, 4);
			modifier.provisionConnectionState(ionet::ServiceIdentifier(888), keys[0]) = CreateConnectionState(0, 5, 4, 3);
			modifier.provisionConnectionState(ionet::ServiceIdentifier(222), keys[0]) = CreateConnectionState(5, 4, 3, 2);

			modifier.provisionConnectionState(ionet::ServiceIdentifier(123), keys[1]) = CreateConnectionState(1, 2, 3, 4);
			modifier.provisionConnectionState(ionet::ServiceIdentifier(987), keys[1]) = CreateConnectionState(49, 64, 36, 25);

			modifier.provisionConnectionState(ionet::ServiceIdentifier(111), keys[2]) = CreateConnectionState(9, 8, 7, 6);
		}
		...
		AssertDiagnosticNodesHandlerWritesCountsInResponseToValidRequest(nodeContainer, expectedPacketSize, [&keys](const auto& context) {
			...
			AssertConnectionState(nodeInfo1, ionet::ServiceIdentifier(123), 7, 6, 5, 4);
			AssertConnectionState(nodeInfo1, ionet::ServiceIdentifier(888), 0, 5, 4, 3);
			AssertConnectionState(nodeInfo1, ionet::ServiceIdentifier(222), 5, 4, 3, 2);
			...
			AssertConnectionState(nodeInfo2, ionet::ServiceIdentifier(123), 1, 2, 3, 4);
			AssertConnectionState(nodeInfo2, ionet::ServiceIdentifier(987), 49, 64, 36, 25 );
			...
			AssertConnectionState(nodeInfo3, ionet::ServiceIdentifier(111), 9, 8, 7, 6);
		});
	}
