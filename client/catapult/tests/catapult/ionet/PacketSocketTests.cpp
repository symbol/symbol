...
namespace catapult { namespace ionet {
	...
	namespace {
		...
		void AssertWriteSuccess(const PacketPayload& payload, const ByteBuffer& expectedPayload) {
			...
			SocketOperationCode writeResult;
			...
			test::SpawnPacketServerWork(pPool->service(), [&payload, &writeResult](const auto& pServerSocket) -> void {
				pServerSocket->write(payload, [&writeResult](auto result) {
					writeResult = result;
				});
			});
			...
			EXPECT_EQ(SocketOperationCode::Success, writeResult);
			EXPECT_EQUAL_BUFFERS(expectedPayload, 0, Buffer_Size, receiveBuffer);
		}
	}

	TEST(PacketSocketTests, WriteSucceedsWhenSocketWriteSucceeds_ZeroBufferPayload) {
		// Arrange: set up payloads
	...
	TEST(PacketSocketTests, WriteSucceedsWhenSocketWriteSucceeds_SingleBufferPayload) {
		// Arrange: set up payloads
	...
	TEST(PacketSocketTests, WriteFailsWhenSocketWriteFails) {
		// Arrange: set up payloads
		...
		//      "client" - reads a payload from the socket
		auto pPool = test::CreateStartedIoServiceThreadPool();
		test::SpawnPacketServerWork(pPool->service(), [&pPayload, &writeResult](const auto& pServerSocket) -> void {
			pServerSocket->close();
			CATAPULT_LOG(debug) << "closed server socket";

			pServerSocket->write(pPayload, [&writeResult](auto result) {
				writeResult = result;
			});
		});
		test::AddClientReadBufferTask(pPool->service(), receiveBuffer);
		...
		// Assert:
		EXPECT_EQ(SocketOperationCode::Write_Error, writeResult);
		EXPECT_EQ(test::ToHexString(ByteBuffer(Buffer_Size)), test::ToHexString(receiveBuffer));
	}

	TEST(PacketSocketTests, WriteFailsWhenClientSocketCloses) {
		// Arrange: set up payloads
		...
		auto pPool = test::CreateStartedIoServiceThreadPool();
		test::SpawnPacketServerWork(pPool->service(), [&payload, &writeResult, &isSocketClosed](const auto& pServerSocket) -> void {
		...
		// Assert:
		test::AssertSocketClosedDuringWrite(writeResult);
	}

	TEST(PacketSocketTests, WriteCanWriteMultipleConsecutivePayloads) {
		// Assert:
		test::AssertWriteCanWriteMultipleConsecutivePayloads([](const auto&, const auto& pSocket) { return pSocket; });
	}
	...
	namespace {
		...
		SendBuffersResult SendBuffers(const std::vector<ByteBuffer>& sendBuffers) {
			...
			auto pPool = test::CreateStartedIoServiceThreadPool();
			test::SpawnPacketServerWork(pPool->service(), [&result](const auto& pServerSocket) -> void {
	...
	TEST(PacketSocketTests, ReadCanProcessSinglePacket) {
	...
	TEST(PacketSocketTests, ReadCanProcessSinglePacketSpanningReads) {
	...
	TEST(PacketSocketTests, ReadCanProcessFirstOfMultiplePacketsInSingleRead) {
	...
	TEST(PacketSocketTests, ReadFailsOnReadError) {
	...
	TEST(PacketSocketTests, ReadFailsWhenClientSocketCloses) {
		// Arrange:
		...
		//      "client" - closes the client socket
		auto pPool = test::CreateStartedIoServiceThreadPool();
		test::SpawnPacketServerWork(pPool->service(), [&readResult, &isSocketClosed](const auto& pServerSocket) -> void {
			WAIT_FOR(isSocketClosed);
			pServerSocket->read([&readResult](auto code, const auto*) {
				readResult = code;
			});
		});
		test::CreateClientSocket(pPool->service())->connect().then([&isSocketClosed](auto&& socketFuture) {
	...
	TEST(PacketSocketTests, ReadAbortsOnMalformedPacket) {
	...
	namespace {
		std::vector<SendBuffersResult> SendBuffersMultiple(const std::vector<ByteBuffer>& sendBuffers) {
			...
			//      "client" - sends all buffers to the socket
			auto pPool = test::CreateStartedIoServiceThreadPool();
			test::SpawnPacketServerWork(pPool->service(), [&results](const auto& pServerSocket) -> void {
	...
	TEST(PacketSocketTests, ReadMultipleCanProcessSinglePacket) {
		// Arrange: send a single buffer containing a single packet
	...
	TEST(PacketSocketTests, ReadMultipleCanProcessSinglePacketSpanningReads) {
		// Arrange: send a packet spanning three buffers
		auto sendBuffers = test::GenerateRandomPacketBuffers({ 50, 50, 50 });
		test::SetPacketAt(sendBuffers[0], 0, 125);
	...
	TEST(PacketSocketTests, ReadMultipleCanProcessMultiplePacketsInSingleRead) {
		// Arrange: send a buffer containing three packets
	...
	TEST(PacketSocketTests, ReadMultipleAbortsOnMalformedPacket) {
	...
	TEST(PacketSocketTests, ReadMultipleCanProcessMultiplePacketsSpanningMultipleReads) {
	...
	TEST(PacketSocketTests, ReadCanReadMultipleConsecutivePayloads) {
		// Assert:
		test::AssertReadCanReadMultipleConsecutivePayloads([](const auto&, const auto& pSocket) { return pSocket; });
	}
	...
	TEST(PacketSocketTests, CloseTransitionsSocketToClosedState) {
		// Arrange:
		auto pPool = test::CreateStartedIoServiceThreadPool();
		auto pAcceptor = test::CreateLocalHostAcceptor(pPool->service());

		// Act: "server" - accepts a connection and closes the socket
		//      "client" - connects to the server
		PacketSocket::Stats stats;
		std::shared_ptr<PacketSocket> pSocket;
		pPool->service().post([pAcceptor, &stats, &pSocket]() -> void {
			Accept(*pAcceptor, test::CreatePacketSocketOptions(), [&stats, &pSocket](const auto& pAcceptSocket) -> void {
				pSocket = pAcceptSocket;
				pSocket->close();
	...
	TEST(PacketSocketTests, CanCloseWithPendingOperations) {
		// Arrange: send one buffer containing one packet
		auto sendBuffer = test::GenerateRandomPacketBuffer(50, { 30 });
		std::vector<ByteBuffer> sendBuffers{ sendBuffer };
	...
	TEST(PacketSocketTests, OperationsOnSingleSocketAreStranded) {
		// Arrange:
		auto pPool = test::CreateStartedIoServiceThreadPool();
		auto pAcceptor = test::CreateLocalHostAcceptor(pPool->service());
		...
		pPool->service().post([pAcceptor, &breadcrumbs, &pSocket]() {
			Accept(*pAcceptor, test::CreatePacketSocketOptions(), [&breadcrumbs, &pSocket](const auto& pAcceptSocket) -> void {
				pSocket = std::move(pAcceptSocket);
	...
	TEST(PacketSocketTests, AcceptReturnsOpenSocketOnSuccess) {
		// Arrange:
		auto pPool = test::CreateStartedIoServiceThreadPool();
		auto pAcceptor = test::CreateLocalHostAcceptor(pPool->service());
		...
		PacketSocket::Stats stats;
		std::shared_ptr<PacketSocket> pSocket;
		pPool->service().post([pAcceptor, &stats, &pSocket]() {
			Accept(*pAcceptor, test::CreatePacketSocketOptions(), [&stats, &pSocket](const auto& pAcceptSocket) -> void {
				pSocket = pAcceptSocket;
				pSocket->stats([&stats](const auto& socketStats) { stats = socketStats; });
			});
	...
	TEST(PacketSocketTests, AcceptReturnsNullSocketOnFailure) {
		// Arrange:
		auto pPool = test::CreateStartedIoServiceThreadPool();
		auto pAcceptor = test::CreateLocalHostAcceptor(pPool->service());
		...
		pPool->service().post([&pPool, pAcceptor, &pSocket, &isCallbackCalled]() {
			auto invalidateSocket = [](auto& socket) { socket.open(boost::asio::ip::tcp::v4()); };
			auto options = test::CreatePacketSocketOptions();
			Accept(*pAcceptor, options, invalidateSocket, [&pSocket, &isCallbackCalled](const auto& pAcceptSocket) {
				pSocket = pAcceptSocket;
	...
	TEST(PacketSocketTests, AcceptCallsConfigureSocketCallbackForSocket) {
		// Arrange:
		auto pPool = test::CreateStartedIoServiceThreadPool();
		auto pAcceptor = test::CreateLocalHostAcceptor(pPool->service());
		...
		pPool->service().post([&pPool, pAcceptor, &numConfigureSocketCalls]() {
			Accept(
					*pAcceptor,
	...
	TEST(PacketSocketTests, AcceptHonorsOptions) {
		// Arrange:
		...
		pPool->service().post([&, pAcceptor]() {
			Accept(*pAcceptor, options, [&result](const auto& pServerSocket) {
				pServerSocket->read([pServerSocket, &result](auto code, const auto* pPacket) {
					FillResult(result, pServerSocket, code, pPacket);
				});
			});
	...
	TEST(PacketSocketTests, ConnectionFailsIfNodeEndpointCannotBeResolved) {
	...
	TEST(PacketSocketTests, ConnectionFailsIfServerRefusesConnection) {
	...
	TEST(PacketSocketTests, ConnectionSucceedsIfServerAcceptsConnection) {
		// Arrange: set up a server thread
		boost::asio::io_service service;
		test::SpawnPacketServerWork(service, [](const auto&) {});
	...
	namespace {
		bool RunCancellationTestIteration() {
			...
			auto pAcceptor = test::CreateLocalHostAcceptor(service);
			auto serverSocket = boost::asio::ip::tcp::socket(service);
			pAcceptor->async_accept(serverSocket, [](const auto& acceptEc) {
	...
	TEST(PacketSocketTests, ConnectionFailsIfConnectAttemptIsCancelled) {
	...
	TEST(PacketSocketTests, ConnectHonorsOptions) {
		// Arrange:
		constexpr auto Buffer_Size = 150u;
		auto sendBuffer = test::GenerateRandomPacketBuffer(Buffer_Size);
		auto pPool = test::CreateStartedIoServiceThreadPool();
		...
		SendBuffersResult result;
		test::SpawnPacketServerWork(pPool->service(), [&sendBuffer](const auto& pServerSocket) -> void {
