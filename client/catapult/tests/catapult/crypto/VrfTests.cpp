...
#undef VRF_PROOF_FIELDS
	...
	namespace {
		...
		static std::vector<TestVectorOutput> SampleTestVectorsOutput() {
			return { { "9275DF67A68C8745C0FF97B48201EE6DB447F7C93B23AE24CDC2400F52FDB08A",
	...
	TEST(TEST_CLASS, VerifyVrfProofFailsWhenVerificationHashIsWrong) {
		...
		AssertVerifyVrfProofFailsWhenProofIsCorrupted([](auto& vrfProof) { vrfProof.VerificationHash[4] ^= 0xFF; });
	}

	TEST(TEST_CLASS, VerifyVrfProofFailsWhenScalarIsWrong) {
		// Act: corrupt Scalar
		AssertVerifyVrfProofFailsWhenProofIsCorrupted([](auto& vrfProof) { vrfProof.Scalar[14] ^= 0xFF; });
	}

	TEST(TEST_CLASS, VerifyVrfProofFailsWhenScalarIsNotReduced) {
		// Act: add group order to Scalar
		AssertVerifyVrfProofFailsWhenProofIsCorrupted([](auto& vrfProof) { test::ScalarAddGroupOrder(vrfProof.Scalar.data()); });
	}
