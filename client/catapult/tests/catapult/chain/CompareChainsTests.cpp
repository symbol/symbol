...
#define HASH_TEST(TEST_NAME)                                         \
	...
	namespace {
		void AssertRemoteIsNotSyncedWhenLocalContainsExactlyFirstBatchButRemoteHasMultipleBatches(uint64_t adjustment) {
			...
			auto expectedHashesFromRequests = std::vector<std::pair<Height, uint32_t>> { { Finalized_Height, Batch_Size },
	...
	TEST(TEST_CLASS, RemoteHasSmallerBatchSizeThanLocal) {
		...
		// - check requests
		auto expectedHashesFromRequests = std::vector<std::pair<Height, uint32_t>> { { Height(1), 1210 }, { Height(2131), 1210 }, { Height(3196), 1210 },
