...
namespace catapult { namespace chain {
	namespace {
		...
		std::vector<std::shared_ptr<state::AccountState>> CreateAccounts(
				cache::AccountStateCacheDelta& cache,
				const std::vector<KeyPair>& keyPairs,
				const std::vector<Importance> importances) {
			std::vector<std::shared_ptr<state::AccountState>> accountStates;
			for (auto i = 0u; i < keyPairs.size(); ++i) {
				auto pState = cache.addAccount(keyPairs[i].publicKey(), Height(1));
				pState->ImportanceInfo.set(importances[i], model::ImportanceHeight(1));
				accountStates.push_back(pState);
			}
		...
		Key BestHarvesterKey(const model::BlockElement& lastBlockElement, const std::vector<KeyPair>& keyPairs) {
			...
			for (const auto& keyPair : keyPairs) {
				...
				uint64_t hit = CalculateHit(generationHash);
				if (hit < bestHit) {
					bestHit = hit;
		...
		Timestamp CalculateBlockGenerationTime(const HarvesterContext& context, const Key& publicKey) {
			...
			auto pState = view->findAccount(publicKey);
			uint64_t hit = CalculateHit(model::CalculateGenerationHash(context.LastBlockElement.GenerationHash, publicKey));
			uint64_t referenceTarget = static_cast<uint64_t>(CalculateTarget(
					utils::TimeSpan::FromMilliseconds(1000),
					difficulty,
					pState->ImportanceInfo.current(),
					config));
	...
	TEST(HarvesterTests, HarvestReturnsNullptrIfNoAccountIsUnlocked) {
		// Arrange:
	...
	TEST(HarvesterTests, HarvestReturnsBlockIfEnoughTimeElapsed) {
		// Arrange:
		HarvesterContext context;
		auto pHarvester = context.CreateHarvester();
	...
	TEST(HarvesterTests, HarvestReturnsNullptrIfDifficultyCacheDoesNotContainInfoAtLastBlockHeight) {
		// Arrange:
		HarvesterContext context;
		auto pHarvester = context.CreateHarvester();
	...
	TEST(HarvesterTests, HarvesterWithBestKeyCreatesBlockAtEarliestMoment) {
		// Arrange:
		...
		test::RunNonDeterministicTest("harvester with best key harvests", []() -> bool {
			HarvesterContext context;
	...
	TEST(HarvesterTests, HarvestReturnsNullptrIfNoHarvesterHasHit) {
		// Arrange:
		HarvesterContext context;
		auto bestKey = BestHarvesterKey(context.LastBlockElement, context.KeyPairs);
		auto timestamp = CalculateBlockGenerationTime(context, bestKey);
	...
	TEST(HarvesterTests, HarvestReturnsNullptrIfNoHarvesterHasImportanceAtBlockHeight) {
		// Arrange:
		HarvesterContext context;
	...
	TEST(HarvesterTests, HarvestReturnsNullptrIfAccountsAreUnlockedButNotFoundInCache) {
		// Arrange:
		HarvesterContext context;
		auto pHarvester = context.CreateHarvester();
	...
	TEST(HarvesterTests, HarvestHasFirstHarvesterWithHitAsSigner) {
		// Arrange:
		HarvesterContext context;
		auto pHarvester = context.CreateHarvester();
	...
	TEST(HarvesterTests, HarvestedBlockHasExpectedProperties) {
		// Arrange:
		// - the harvester accepts the first account that has a hit. That means that subsequent accounts might have
		// - a better (lower) hit but still won't be the signer of the block.
		test::RunNonDeterministicTest("harvested block has expected properties", []() -> bool {
			HarvesterContext context;
			auto pLastBlock = context.pLastBlock;
			auto bestKey = BestHarvesterKey(context.LastBlockElement, context.KeyPairs);
			auto timestamp = CalculateBlockGenerationTime(context, bestKey);
			...
			EXPECT_EQ(CalculateDifficulty(difficultyCache, pLastBlock->Height, config), pBlock->Difficulty);
			EXPECT_EQ(model::MakeVersion(Network_Identifier, 3), pBlock->Version);
			EXPECT_EQ(model::EntityType::Block, pBlock->Type);
			EXPECT_TRUE(model::IsSizeValid(*pBlock, *std::make_shared<model::TransactionRegistry>()));
			return true;
		});
	}

	TEST(HarvesterTests, HarvesterRespectsCustomBlockChainConfiguration) {
	...
	TEST(HarvesterTests, HarvestUsesTransactionSupplier) {
		// Arrange:
		HarvesterContext context;
		size_t counter = 0u;
		auto pHarvester = context.CreateHarvester(
	...
	TEST(HarvesterTests, HarvestPutsNoTransactionsInBlockIfCacheIsEmpty) {
		// Assert: numAvailableTransactions / maxTransactionsPerBlock / numExpectedTransactionsInBlock
		AssertTransactionsInBlock(0, 10, 0);
	}

	TEST(HarvesterTests, HarvestPutsAllTransactionsFromCacheIntoBlockIfAllAreRequested) {
		// Assert: numAvailableTransactions / maxTransactionsPerBlock / numExpectedTransactionsInBlock
		AssertTransactionsInBlock(5, 10, 5);
	}

	TEST(HarvesterTests, HarvestPutsMaxTransactionsIntoBlockIfMaxTransactionsAreRequestedAndCacheHasEnoughTransactions) {
		// Assert: numAvailableTransactions / maxTransactionsPerBlock / numExpectedTransactionsInBlock
