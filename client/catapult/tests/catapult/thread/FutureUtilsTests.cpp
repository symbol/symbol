...
#define WHEN_ALL_TEST(TEST_NAME)                                 \
	...
	TEST(TEST_CLASS, ComposeDoesNotBlock) {
		// Arrange:
		auto composedFuture = compose(CreateSleepValueFuture(25, 7), [](auto&& future) { return CreateSleepContinuationFuture(25, std::move(future)); });
	...
	TEST(TEST_CLASS, CanComposeNonExceptionalFutures) {
		// Arrange:
		auto composedFuture = compose(CreateSleepValueFuture(15, 7), [](auto&& future) { return CreateSleepContinuationFuture(10, std::move(future)); });
	...
	TEST(TEST_CLASS, CanComposeFuturesWhenFirstIsExceptional) {
		// Arrange:
		auto composedFuture = compose(CreateSleepExceptionFuture(15), [](auto&& future) { return CreateSleepContinuationFuture(10, std::move(future)); });
