...
namespace catapult { namespace mocks {
	...
	MemoryBasedStorage::MemoryBasedStorage() {
	...
	Height MemoryBasedStorage::chainHeight() const {
		return m_height;
	}
	...
	void MemoryBasedStorage::saveBlock(const model::BlockElement& blockElement) {
	...
	std::shared_ptr<const model::Block> MemoryBasedStorage::loadBlock(Height height) const {
		if (height > m_height)
	...
	std::shared_ptr<const model::BlockElement> MemoryBasedStorage::loadBlockElement(Height height) const {
		if (height > m_height)
			CATAPULT_THROW_INVALID_ARGUMENT_1("cannot load block at height greater than chain height", height);
	...
	model::HashRange MemoryBasedStorage::loadHashesFrom(Height height, size_t maxHashes) const {
	...
	void MemoryBasedStorage::dropBlocksAfter(Height height) {
		m_height = height;
	}

	std::unique_ptr<io::BlockStorage> CreateMemoryBasedStorage(uint32_t numBlocks) {
		auto pStorage = std::make_unique<MemoryBasedStorage>();
